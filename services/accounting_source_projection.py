...
def _missing_terms(source: dict[str, Any]) -> list[str]:
    """List required pricing terms that are absent from canonical sources."""
    missing = []
...
def load_case_accounting_source_with_cursor(cursor: Any, case_no: str) -> dict[str, Any]:
    """Load canonical source facts using the caller's transaction cursor.
    ...
    historic payment records.  This function does not own or mutate the cursor
    lifecycle, so callers may safely use it inside an existing transaction.
    """
    ...
    source = {
        ...
        "collection_schedule": {
            "deposit_service_days": case_row.get("deposit_service_days"),
            "deposit_due_date": case_row.get("deposit_due_date"),
        },
    }

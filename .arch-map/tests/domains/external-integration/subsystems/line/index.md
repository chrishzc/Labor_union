...
- `tests/test_line_admin_capabilities_contract.py` remains relocation-sensitive because it derives the repository root from `__file__` to inspect the LINE admin route source.
...
# Flat-test audit
The current flat-test audit found no additional high-confidence LINE owner-local tests outside the documented boundary/release/relocation-sensitive classes. Admit future cases by direct SUT/current ownership rather than filename alone.

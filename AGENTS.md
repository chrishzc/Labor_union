...
- 使用者只是詢問問題時，只能分析並回答；不得自行提出實作計畫、修改檔案、執行 migration
  或改變外部狀態。
- 使用者要求計畫時，必須先閱讀 `document/` 內相關正式架構，再依序提供：
  1. 簡顯易懂但完整的修改目的與功能／Bug 摘要。
  2. 符合正式文件的 `Global → Domain → Subsystem → Module` 架構與流程。
  3. 預計修改、新增、刪除的檔案清單及各檔責任。
- 計畫提出後必須等待使用者明確批准；未批准前不得修改 production code、test、script、
  schema、設定或文件。使用者直接要求修改但尚無已批准計畫時，也必須先提出計畫等待批准。
- 實作完成後必須清除只供開發使用的一次性程式、暫存檔與產物，確認沒有污染工作區，並將
  當次成果與實際驗證證據寫入 `history/work_log.md`；不得自行執行 `git add` 或 commit。
- 只有使用者明確同意 commit 後，才可以先更新 `history/git_push.md`，完成後依序執行
  `git add` 與 commit；未取得 commit 同意時不得 stage、commit 或 push。

    ...
    try:
        ...
        try:
            with conn.cursor() as cursor:
                cursor.execute("""
                    SELECT m.id, m.order_id, m.staff_id, s.name, s.line_user_id, c.name AS client_name, o.case_no
...
@router.post("/orders/{order_id}/assign-staff", response_model=BaseResponse[bool])
def assign_staff_to_order(
    ...
    """成立訂單並定案指派服務人員/月嫂"""
    try:
        success = db_service.assign_staff_to_order(order_id, req.staff_id)

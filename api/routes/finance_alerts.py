"""Typed finance-alert read and workflow endpoints."""
...
assert router.prefix == "/api/v1/finance-alerts"

# Backwards-compatible import names; validation ownership is the shared contract.
ClaimFinanceAlertRequest = ClaimAlertRequest
ResolveFinanceAlertRequest = ResolveAlertRequest
...
def _workflow_error(exc: ValueError) -> HTTPException:
    ...
    if message == "alert_id does not exist":
        return _typed_http_error(
            404, TypedErrorCode.NOT_FOUND, "找不到指定的帳務警示"
        )
    return _typed_http_error(
...
def list_alerts(
    ...
    try:
        ...
        items = [
            alert_summary_from_record(record, AlertFamily.FINANCE)
        ...
        return AlertCenterResponse(data=data, message="帳務警示讀取完成")
...
@router.get(
    "/{alert_id}",
    response_model=AlertCenterResponse[FinanceAlertDetailViewModel],
)
def get_alert(alert_id: int = Path(..., ge=1)):
    connection = get_connection()
    try:
        with connection.cursor() as cursor:
            record = get_finance_alert(cursor, alert_id)
        if record is None:
            raise _typed_http_error(
                404, TypedErrorCode.NOT_FOUND, "找不到指定的帳務警示"
            )
        return AlertCenterResponse(
            data=finance_alert_detail_from_record(record),
            message="帳務警示詳情讀取完成",
        )
    except HTTPException:
        raise

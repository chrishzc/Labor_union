...
Description: 提供受控檔案 staging、Preview、Apply、list、detail、download 與 receipt API。
"""
...
@router.post("/files/apply", response_model=BaseResponse[ControlledFileReceiptView])
...
@router.get("/files", response_model=BaseResponse[ControlledFileListView])
def list_controlled_files(
    ...
    return _call_workflow(
        lambda: BaseResponse(
            data=ControlledFileListView(
                items=tuple(_detail_view(item) for item in workflow.list_readbacks())
            ),
            message="成功取得受控檔案清單",
        ),
...
@router.get("/files/{file_id}", response_model=BaseResponse[ControlledFileDetailView])
...
def _workflow_http_error(error: ControlledFileWorkflowError, correlation_id: str):
    code = error.code

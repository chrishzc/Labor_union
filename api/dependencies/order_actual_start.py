...
class ActualStartApplication:
    ...
    def preview(self, case_no, new_actual_start_date):
        source = self._restarted_historical_source(case_no, new_actual_start_date)
    ...
    def apply(self, request: ActualStartApplyRequest):
    ...
    def _restarted_historical_source(self, case_no, actual_start_date):
        if not self._is_restarted_historical_tombstone(case_no):
            return None
    ...
    def _is_restarted_historical_tombstone(self, case_no):
        if self.restarted_historical_source is None:
            return False
        facts = self.repository.load_for_preview(case_no).shared_facts
        return (
            facts.lifecycle.historical_precision_restarted
            and not facts.scheduling.segments
        )

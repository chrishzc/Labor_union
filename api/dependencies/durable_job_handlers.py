"""
File: durable_job_handlers.py
Description: API composition for concrete durable-job handlers.

The durable worker remains a generic subsystem. This module owns the concrete
application, repository, and connection composition required by each command.
"""
...
from subsystems.jobs.durable_job_worker import JobHandler
...
def default_job_handlers() -> dict[str, JobHandler]:
    return {
        "anomaly.recheck": anomaly_recheck_handler,
...
def anomaly_recheck_handler(payload: dict[str, Any]) -> tuple[dict[str, Any], str]:
    """Run one composed current-only anomaly recheck.

    Owner snapshot readers and detectors are deliberately supplied by the
    anomaly runtime composition.  An unconfigured process fails as retryable
    ...
    try:
        try:
            ...
            raise RetryableDurableJobError(
                ...
                "Anomaly recheck owner composition is unavailable.",
            ) from error
        identity = payload.get("intent_identity", "unknown")
        return _materialize(result), "anomaly-recheck:" + str(identity)
    finally:
        connection.close()

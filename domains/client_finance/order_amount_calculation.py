...
SUBSIDIZED_EXCESS_CLIENT_HOURLY_RATE = Decimal("350")
...
def _collection_stages(
    *,
    ...
    identity_status: str,
    standard_client_rate: Decimal,
    subsidized_excess_hourly_rate: Decimal,
    ...
    remaining_subsidy_hours = None
    if identity_status == "補助市民":
        remaining_subsidy_hours = Decimal("120")
    stages = []
    for stage, days, floor_fee in (
        ...
        service_hours = days * hours_per_day
        service_receivable, remaining_subsidy_hours = _stage_service_receivable(
            identity_status,
            service_hours,
            standard_client_rate,
            subsidized_excess_hourly_rate,
            remaining_subsidy_hours,
        )
        receivable = service_receivable + floor_fee
        stages.append({
            "stage": stage,
...
def _stage_service_receivable(
    identity_status: str,
    service_hours: Decimal,
    standard_client_rate: Decimal,
    subsidized_excess_hourly_rate: Decimal,
    remaining_subsidy_hours: Decimal | None,
) -> tuple[Decimal, Decimal | None]:
    if identity_status != "補助市民":
        return service_hours * standard_client_rate, remaining_subsidy_hours
    if remaining_subsidy_hours is None:
        raise ValueError("subsidy coverage state is missing")
    covered_hours = min(service_hours, remaining_subsidy_hours)
    excess_hours = service_hours - covered_hours
    return (
        excess_hours * subsidized_excess_hourly_rate,
        remaining_subsidy_hours - covered_hours,
    )
...
def _subsidy_claim_amount(
    subsidy_hours: Decimal,
    subsidy_claim_hourly_rate: Decimal,
    staff_plans: list[dict[str, Any]],
    ...
    for plan in eligible:
        ...
        allocated_amount = allocated_hours * subsidy_claim_hourly_rate
        claim_amount += allocated_amount
        allocations.append({
            "assignment_id": plan["assignment_id"],
            "staff_id": plan["staff_id"],
            "subsidy_hours": _number(allocated_hours),
            "service_unit_price": _number(subsidy_claim_hourly_rate),
...
def calculate_order_amounts(
    ...
    if policy_identity_status not in SUBSIDY_HOURS_BY_IDENTITY_STATUS:
        raise ValueError("unsupported identity_status")
    standard_client_rate = CLIENT_RATE_BY_IDENTITY_STATUS[policy_identity_status]
    ...
    coverage = derive_subsidy_coverage(identity_status, total_hours, client_floor_fee)
    ...
    stages = _collection_stages(
        ...
        standard_client_rate=standard_client_rate,
        subsidized_excess_hourly_rate=SUBSIDIZED_EXCESS_CLIENT_HOURLY_RATE,
    ...
    client_prepaid_subsidy = subsidy_hours * standard_client_rate
    staff_due_date = (
    ...
    claim_amount, claim_allocations = _subsidy_claim_amount(
        subsidy_hours,
        coverage.subsidy_claim_hourly_rate,
        staff_plans,
    )
    ...
    return {
        ...
        "subsidy_coverage": {
            "subsidy_hours": _number(coverage.subsidy_hours),
            "self_pay_service_hours": _number(coverage.self_pay_service_hours),
            "subsidy_claim_hourly_rate": _number(coverage.subsidy_claim_hourly_rate),
            "is_full_subsidy_order": coverage.is_full_subsidy_order,
        },
        "client_ledger_plan": {
            "stages": stages,

...
def test_each_order_status_has_its_own_count_and_xlsx_column(status):
    report = _query(_Facts([_case("2026-09-01", 1, order_status=status)]))
    expected = {label: int(label == status) for label in ORDER_STATUSES}
    for total in (report.summary, report.annual_totals[0], report.monthly_subtotals[0]):
        assert total.order_status_counts == expected
        assert total.order_status_missing_count == 0
        assert sum(total.order_status_counts.values()) == total.application_count == 1

    sheet = _workbook(report).worksheets[0]
    assert tuple(sheet.cell(3, col).value for col in range(13, 23)) == ORDER_STATUSES
    status_column = 13 + ORDER_STATUSES.index(status)
    detail_row = next(r for r in range(5, sheet.max_row + 1) if sheet.cell(r, 1).value == 1)
    august_row = next(r for r in range(5, sheet.max_row + 1) if sheet.cell(r, 1).value == "115年8月小計")
    for r in (4, detail_row, august_row):
        assert sheet.cell(r, status_column).value == 1
        assert sum(sheet.cell(r, col).value or 0 for col in range(13, 24)) == 1
    assert sheet.max_column == 31
    assert sheet.cell(2, 23).value == "無訂單／狀態缺值"
    assert sheet.cell(2, 24).value == "審核不符合"
    assert sheet.cell(detail_row, 25).value == 20
    assert sheet.cell(detail_row, 26).value == 8
    assert {"M2:V2", "W2:W3", "X2:X3"}.issubset({str(r) for r in sheet.merged_cells.ranges})


def test_status_counts_and_missing_reconcile_without_double_counting_rejection():
    cases = [_case("2026-09-01", index, order_status=status)
             for index, status in enumerate(ORDER_STATUSES, 1)]
    cases.extend([
        _case("2026-01-06", 11, order_status="待補件"),
        _case("2026-09-02", 12, order_status=None, identity_status=None, reject_reason="測試未分流"),
        _case("2026-09-07", 13, order_status="訂單取消", reject_reason="測試審核不符合"),
    ])
    report = _query(_Facts(cases))
    annual = report.annual_totals[0]
    august, september = report.monthly_subtotals
    assert annual.application_count == 13 and report.summary.application_count == 12
    assert annual.order_status_counts["待補件"] == 2
    assert august.order_status_counts == dict.fromkeys(ORDER_STATUSES, 1)
    assert september.order_status_counts["訂單取消"] == 1
    assert annual.review_rejected_count == 2
    assert annual.order_status_missing_count == 1
    for total in (report.summary, annual, august, september):
        assert sum(total.order_status_counts.values()) + total.order_status_missing_count == total.application_count

    sheet = _workbook(report).worksheets[0]
    for label, total in (("115年度累計", annual), ("115年8月小計", august), ("115年9月小計", september)):
        r = next(r for r in range(4, sheet.max_row + 1) if sheet.cell(r, 1).value == label)
        assert [sheet.cell(r, col).value for col in range(13, 23)] == list(total.order_status_counts.values())
        assert sheet.cell(r, 23).value == total.order_status_missing_count
        assert sheet.cell(r, 24).value == total.review_rejected_count
        assert sum(sheet.cell(r, col).value for col in range(13, 24)) == sheet.cell(r, 8).value


def test_named_source_status_is_not_silently_dropped_even_if_only_in_year_to_date():
    report = _query(_Facts([
        _case("2026-01-06", 1, order_status="來源具名狀態"),
        _case("2026-09-01", 2, order_status="服務中"),
    ]))
    sheet = _workbook(report).worksheets[0]
    assert sheet.cell(3, 23).value == "來源具名狀態"
    assert sheet.cell(4, 23).value == 1
    august_row = next(r for r in range(5, sheet.max_row + 1) if sheet.cell(r, 1).value == "115年8月小計")
    assert sheet.cell(august_row, 23).value == 0
    assert sheet.cell(2, 24).value == "無訂單／狀態缺值"
    assert sheet.cell(2, 25).value == "審核不符合"


def test_empty_report_still_has_all_order_status_columns_with_zero_totals():
    report = _query(_Facts())
    for total in (report.summary, *report.annual_totals, *report.monthly_subtotals):
        assert total.order_status_counts == dict.fromkeys(ORDER_STATUSES, 0)
        assert total.order_status_missing_count == 0
    sheet = _workbook(report).worksheets[0]
    assert tuple(sheet.cell(3, col).value for col in range(13, 23)) == ORDER_STATUSES
    assert [sheet.cell(4, col).value for col in range(13, 24)] == [0] * 11


@pytest.mark.parametrize("invalid", [-1, 1.5, "1", True])
def test_status_counts_reject_invalid_api_values(invalid):
    total = asdict(_query(_Facts()).annual_totals[0])
    total["order_status_counts"]["待補件"] = invalid
    with pytest.raises(ValidationError):

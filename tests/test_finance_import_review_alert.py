...
def test_project_creates_import_alert_for_occurrence_integrity_mismatch(monkeypatch):
    ...
    monkeypatch.setattr(
        subject,
        "_project_canonical_import006_alert",
        lambda *args, **kwargs: {
            "action": "created",
    ...
    assert result["alert"]["fingerprint"] == "a" * 64


def test_project_keeps_consistent_batch_without_active_alert(monkeypatch):
    cursor = ScriptedCursor(
        [
...
def test_summary_event_identity_is_bounded_and_deterministic():
    first = subject._summary_event_identity(4, True, {"remaining_count": 1})
    second = subject._summary_event_identity(4, True, {"remaining_count": 1})
...
def test_project_also_mirrors_into_canonical_anomaly_registry(monkeypatch):
    ...
    def _fake_canonical_projection(
        _cursor,
        batch_id,
        summary,
        details,
        *,
        ...
        captured["source_event_identity"] = source_event_identity
        return {"action": "created", "alert": {"fingerprint": "b" * 64}}
    ...
    assert captured["source_version"] == 0
    assert captured["source_event_identity"] is None

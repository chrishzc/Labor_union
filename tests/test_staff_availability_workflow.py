"""
File: test_staff_availability_workflow.py
Description: 驗證不可服務期間的 fresh lock、衝突、重播與 append-only 不變量。
"""
...
class FakeRepository:
    ...
    def lock_staff_occupancy_mutex(self, staff_id):
        self.occupancy_locks.append(staff_id)
...
class _FakeUnitOfWork(AbstractContextManager):
    """Minimal outer transaction owner; repository must not commit or rollback."""
...
def test_create_pause_apply_is_atomic_and_exact_replay_returns_same_receipt():
    ...
    assert repository.unit_of_work.commit_count == 2
    assert repository.unit_of_work.rollback_count == 0
    assert repository.occupancy_locks == [7, 7]
...
def test_apply_rejects_stale_version_without_commit():
    ...
    assert repository.unit_of_work.commit_count == 0
    assert repository.unit_of_work.rollback_count == 1
...
def test_same_idempotency_key_with_different_preview_is_rejected():
    repository = FakeRepository()
    workflow = _workflow(repository)
    intent = _pause_intent()
    preview = workflow.preview(StaffAvailabilityPreviewRequest(intent))
    workflow.apply(_apply_request(intent, preview.preview_fingerprint, 0, "preview-key"))

    with pytest.raises(StaffAvailabilityDomainError) as caught:
        workflow.apply(
            _apply_request(intent, PreviewFingerprint("f" * 64), 0, "preview-key")
        )

    assert caught.value.code is StaffAvailabilityErrorCode.IDEMPOTENCY_CONFLICT


def test_receipt_race_rolls_back_then_replays_immutable_receipt_or_rejects_mismatch():
    repository = _ReceiptRaceRepository()
    first_unit, race_replay_unit, normal_replay_unit, mismatch_unit = (
        _FakeUnitOfWork() for _ in range(4)
    )
    units = [first_unit, race_replay_unit, normal_replay_unit, mismatch_unit]
    workflow = _workflow_with_units(repository, units)
    intent = _pause_intent()
    preview = workflow.preview(StaffAvailabilityPreviewRequest(intent))
    original = _apply_request(intent, preview.preview_fingerprint, 0, "race-key")

    receipt = workflow.apply(original)
    replay = workflow.apply(original)

    assert replay == receipt
    assert first_unit.commit_count == 0
    assert first_unit.rollback_count == 1
    assert race_replay_unit.commit_count == 1
    assert race_replay_unit.rollback_count == 0
    assert normal_replay_unit.commit_count == 1
    assert len(repository.events) == 1
    ...
    with pytest.raises(StaffAvailabilityDomainError) as caught:
        workflow.apply(
            _apply_request(changed, PreviewFingerprint("f" * 64), 0, "race-key")
        )

    assert caught.value.code is StaffAvailabilityErrorCode.IDEMPOTENCY_CONFLICT
    assert mismatch_unit.commit_count == 0
    assert mismatch_unit.rollback_count == 1
...
def test_cancel_preserves_block_as_cancelled_audit_fact():
    ...
    assert repository.events[0][1].start_date == date(2026, 10, 1)
    assert repository.events[0][1].end_date is None


def test_availability_workflow_does_not_own_database_commit_or_rollback():
    source = Path(
        "subsystems/scheduling/staff_availability_workflow.py"
    ).read_text(encoding="utf-8")

    assert "self._repository.commit(" not in source
    assert "self._repository.rollback(" not in source
    assert "lock_staff_occupancy_mutex" in source


def _workflow(repository):
    clock = FixedBusinessClock(datetime(2026, 8, 13, 9, 0, tzinfo=TAIPEI_TIME_ZONE))
    return StaffAvailabilityWorkflow(repository, clock, lambda: repository.unit_of_work)


def _workflow_with_units(repository, units):
    clock = FixedBusinessClock(datetime(2026, 8, 13, 9, 0, tzinfo=TAIPEI_TIME_ZONE))
    return StaffAvailabilityWorkflow(repository, clock, lambda: units.pop(0))
...
class _ReceiptRaceRepository(FakeRepository):
    def __init__(self):
        super().__init__()
    ...
    def save_receipt(self, request, request_fingerprint, receipt, occurred_at):
        if self._race_once:
            ...
            raise StaffAvailabilityReceiptRaceError("receipt race")
        super().save_receipt(request, request_fingerprint, receipt, occurred_at)

...
from ui.components.contract_evidence_manager import _evidence_rows
...
def test_domain_clients_use_bounded_api_routes_and_forward_operation_headers() -> None:
    ...
    contract = ContractIntegrationApiClient(transport)
    knowledge = KnowledgeRetrievalApiClient(transport)
    runtime = RuntimeHealthApiClient(transport)

    contract.evidence("session", processing_status="normalized", cursor=42)
    contract.map_evidence(
        "session",
        {"provider_contract_id": "provider-1"},
    ...
    assert transport.calls[0]["path"] == "/api/v1/contract-integration/evidence"
    assert transport.calls[0]["params"]["cursor"] == 42
    assert transport.calls[1]["path"] == "/api/v1/contract-integration/mappings"
    assert transport.calls[1]["extra_headers"]["Idempotency-Key"] == "idem-1"
    assert transport.calls[2]["path"] == "/api/v1/knowledge/items/7/publish"
    assert transport.calls[2]["extra_headers"]["X-Correlation-ID"] == "corr-2"
    assert transport.calls[3]["path"] == "/api/v1/runtime/health-status"
...
def test_line_management_ui_has_no_database_or_domain_bypass() -> None:
    ...
    paths.extend(
        [
            ROOT / "ui/components/contract_evidence_manager.py",
...
def test_component_read_models_hide_sensitive_transport_values() -> None:
    evidence = _evidence_rows(
        [
            {
                "provider_contract_id": "external-7",
                "provider_event_id": "secret-event-reference",
                "event_type": "completed",
                "contract_status": "completed",
                "provider_occurred_at": "2026-08-09T00:00:00Z",
                "processing_status": "normalized",
                "internal_contract_identity": None,
    ...
    assert "provider_event_id" not in evidence[0]
    assert "secret-event-reference" not in evidence[0].values()
...
def test_stage9_management_routes_are_registered() -> None:
    ...
    assert "get" in paths["/api/v1/admin/audit"]

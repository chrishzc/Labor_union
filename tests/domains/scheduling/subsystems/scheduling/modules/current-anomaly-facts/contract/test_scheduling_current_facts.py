...
def test_replacement_requires_every_owner_completion_fact() -> None:
    fact = SchedulingReplacementCurrentFact(7, "CASE-7", False, "snapshot-7", 3, True, True, False, True, False, True)
...
def test_overlap_identity_and_recheck_scope_are_canonical() -> None:
    fact = SchedulingOverlapCurrentFact(7, 9, "CASE-7", "CASE-9", "snapshot-overlap", 4, True, True)
...
def test_incomplete_readback_is_always_active() -> None:
    fact = SchedulingOverlapCurrentFact(1, 2, None, None, "snapshot-incomplete", 1, False, False)

...
MANIFEST = ROOT / "db/migration_releases/labor_union_2026_09_22_order_terms_optional_scheduling_receipt_v1.json"
ARTIFACT = "1045_order_terms_optional_scheduling_receipt.sql"
        ...
        "columns": [
            {
                ...
                "column_name": "scheduling_command_receipt_id",
                "column_type": "bigint",
...
def test_fresh_schema_and_preserve_bridge_share_nullable_contract():
    fresh = (ROOT / "db/schema_parts/228_order_terms_optional_scheduling_receipt.sql").read_text(
        encoding="utf-8"
    )
    bridge = (ROOT / "db/schema_parts/1045_order_terms_optional_scheduling_receipt.sql").read_text(
        encoding="utf-8"
    )

    for source in (fresh, bridge):
        assert "scheduling_command_receipt_id BIGINT NULL" in source
...
def test_release_descriptor_classifies_predecessor_successor_and_drift():
    ...
    drift["columns"][0]["column_type"] = "bigint unsigned"
    assert migration.local_additive_descriptor_state(

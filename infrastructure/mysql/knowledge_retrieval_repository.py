...
class MySqlKnowledgeRetrievalRepository:
    ...
    def ingest(self, command):
        ...
        try:
            with self._connection.cursor() as cursor:
                cursor.execute(
                    "INSERT INTO knowledge_items (source_identity,title,source_digest,source_uri,created_by_actor_id) "
                    "VALUES (%s,%s,%s,%s,%s)",
                    (command.source_identity, command.title, digest, command.source_uri, command.actor.actor_id),
                )
                item_id = int(cursor.lastrowid)
                cursor.execute(_INSERT_VERSION, (
                    item_id, 1, command.content, digest, "ingested", command.actor.actor_id,
            ...
            return self._revise_existing(command, digest), True
    ...
    def _transition(self, command, target, event_type):
        with self._connection.cursor() as cursor:
            ...
            if int(item["current_version"]) != command.expected_version.value:
                raise RuntimeError("knowledge_item_version_conflict")
            transition_item_status(KnowledgeItemStatus(item["lifecycle_status"]), target)
            ...
            cursor.execute(_INSERT_VERSION, (
                ...
                event_type, command.actor.actor_id, command.reason, command.idempotency_key.value,
            ))
            cursor.execute(
                "UPDATE knowledge_items SET lifecycle_status=%s,current_version=%s WHERE id=%s AND current_version=%s",
                (target.value, next_version, command.item_id, command.expected_version.value),
            )
        return next_version
    ...
    def _existing_ingest(self, identity, digest):
        with self._connection.cursor() as cursor:
            cursor.execute("SELECT id,source_digest FROM knowledge_items WHERE source_identity=%s", (identity,))
            row = cursor.fetchone()
        if row is None or row["source_digest"] != digest:
            return None
        return int(row["id"])

    # New source version and index staleness are one consistency boundary.
    def _revise_existing(self, command, digest):
        with self._connection.cursor() as cursor:
            cursor.execute(
                "SELECT id,current_version FROM knowledge_items WHERE source_identity=%s FOR UPDATE",
            ...
            cursor.execute(_INSERT_VERSION, (
                ...
                command.actor.actor_id, None, command.idempotency_key.value,
            ))
            cursor.execute(
                "UPDATE knowledge_items SET title=%s,source_digest=%s,source_uri=%s,"
                "lifecycle_status='draft',current_version=%s WHERE id=%s",
                (command.title, digest, command.source_uri, next_version, row["id"]),
            )
...
(item_id,item_version,content,source_digest,event_type,actor_id,reason,idempotency_key)
VALUES (%s,%s,%s,%s,%s,%s,%s,%s)"""
_LOCK_ITEM = "SELECT id,lifecycle_status,current_version FROM knowledge_items WHERE id=%s FOR UPDATE"
...
_PUBLISHED_ITEMS = """SELECT i.source_identity,i.title,i.current_version AS source_version,

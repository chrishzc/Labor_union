...
def _task(row) -> ImportWarningTask:
    ...
    return ImportWarningTask(str(row["occurrence_identity"]), str(row["owning_lane"]), str(row["logical_code"]), str(row["field_path"]), str(row["subject"]), tuple(str(item) for item in issue_codes), ImportWarningTrackingStatus(str(row["tracking_status"])), int(row["tracking_version"]), None, source_receipt_identity=None if row["source_receipt_identity"] is None else str(row["source_receipt_identity"]))
...
_TASK_FIELDS = "o.occurrence_identity,o.owning_lane,o.logical_code,o.field_path,o.subject,o.issue_codes,t.tracking_status,t.tracking_version,o.source_receipt_identity"

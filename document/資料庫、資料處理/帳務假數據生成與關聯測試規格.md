...
- **警示判斷暫緩原則 (Deferred Alert Detection)**：
  假資料生成器**僅負責佈置原始 Fact/交易列**（如重複匯入暫存列與大於應收金額之入款列）。本階段**不得寫入或呼叫** `finance_alerts`、`finance_alert_events`、警示偵測器或警示工作流；警示判斷與生命週期驗證列為後續待辦，播種後兩張警示表均應為 0 筆。
...
5. 驗證 `finance_alerts` 與 `finance_alert_events` 均為 0 筆；警示判斷與其生命週期驗證列為後續待辦，不屬於本次播種驗收。

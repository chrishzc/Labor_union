...
| **身分資格** | `identity_| Excel 欄位名稱 (中文) | 資料庫欄位名 (English) | 資料型態 (MySQL) | 允許空值 | 資料清洗與業務說明 |
| :--- | :--- | :--- | :---: | :--- |
| **項次** | `seq_num` | `INT` | 是 | 原始名冊序號。 |
| **查詢序號** | `query_no` | `VARCHAR(50)` | 否 (UNIQUE) | BeClass 唯一查詢序號，與 `clients.case_no` 進行主關聯，案件識別一律以 clients.case_no 為準。 |

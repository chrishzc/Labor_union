...
本規格書基於 [[自動化系統設計規格書(總覽)]] 與現有 `db/schema.sql`、`scripts/import_excel.py`，定義新竹市月子照顧服務人員職業工會系統中，Excel 來源表單與 MySQL 資料庫之間的欄位對照關係、資料字典結構以及資料清洗/轉換 (ETL) 邏輯。
...
| **案件狀態** | `status` | `VARCHAR(50)` | 是 | - | 例如：「符合」、「不符合」。 |
...
### 5.1 ⚠️ 缺失欄位一：可服務週間與休假偏好 (`weekly_rest_days`)
* **Excel 現有欄位**：服務人員分頁中第 38-42 欄：`可服務週間`（包含 `連續服務`、`週休1日`、`週休2日`、`[其它].3` 複選）。
* **現有 Schema 缺陷**：`db/schema.sql` 的 `staff` 表目前完全沒有儲存此資訊，導致排班演算法無法自動排除週日或週末。
* **修改建議**：
  * 於 `staff` 主表新增欄位 `weekly_rest_days` (`VARCHAR(100)` 或 `JSON`，例如：儲存 `["Sunday"]` 或是 `["Saturday", "Sunday"]`)。
  * 或者，建立 1:N 子表 `staff_rest_days`。為簡化起見，建議在 `staff` 主表中直接新增 `weekly_rest_days` JSON 欄位。

### 5.2 ⚠️ 缺失欄位二：可承接的胎數 (`baby_capacity` / `care_twins`)
* **Excel 現有欄位**：服務人員分頁中第 43-46 欄：`可承接的胎數`（包含 `單胞胎`、`雙胞胎`、`[其它].4` 複選）。
* **現有 Schema 缺陷**：`db/schema.sql` 的 `staff` 表與 `import_excel.py` 目前完全忽略此欄位，若客戶有「雙胞胎」需求，配對功能將無法精準篩選能照顧雙胞胎的月嫂。
* **修改建議**：
  * 於 `staff` 表新增 `care_twins` (`BOOLEAN` 預設為 `FALSE`)，或新增子表 `staff_baby_capacities` 以因應未來三胞胎等特殊需求。

### 5.3 ⚠️ 缺失欄位三：服務人員的 LINE 唯一識別碼 (`line_user_id`)
* **現有 Schema 缺陷**：`db/schema.sql` 的 `staff` 表與 `clients` 表僅有 `line_id` (一般由用戶填寫的搜尋 ID)，但 LINE Messaging API 推播時必須使用 **`line_user_id`** (LINE 內部以 `U` 開頭的 32 碼唯一標識符)。
* **修改建議**：
  * 在 `clients` 表與 `staff` 表分別新增 `line_user_id` (`VARCHAR(50)`，建立 Unique 索引)。當用戶加入好友並完成绑定時，由 FastAPI 接收 Webhook 並回寫此欄位。

...
本系統地端 MySQL 資料庫名稱為 `union_db`。目前 `db/schema.sql` 定義 31 張實體資料表與 1 個檢視表；下表須與 Schema 保持同步：
...
| **`line_confirmation_requests`** | 月嫂驗證與客戶重新綁定人工確認請求 | LINE／LIFF 確認流程 | `id` |
...
- `holidays`：保存國定假日與預設雙倍薪資設定。
- `staff_schedule`：保存每位月嫂每日是否上班、是否雙倍薪資及行政備註。

...
### 5.2 ⚠️ 擴充欄位二：可承接的最大照顧寶寶數量 (`care_babies`)
* **Excel 現有欄位**：服務人員分頁中第 43-46 欄：`可承接的胎數`（包含 `單胞胎`、`雙胞胎`、`[其它].4` 複選）。
* **現有 Schema 缺漏**：`db/schema.sql` 的 `staff` 表目前尚未包含此欄位，導致 Data Pipeline 匯入及媒合篩選時無法精準辨識月嫂照顧雙胞胎以上之能力。
* **修改與 Data Pipeline 更新採納**：
  * 於 `staff` 主表新增欄位 `care_babies` (`INT DEFAULT 1 COMMENT '最大可照顧寶寶數量 (1:單胞胎, 2:雙胞胎, 3:三胞胎)'`)。
  * **Data Pipeline 同步更新**：在 `scripts/imports/import_staff_beclass.py` 中，解析 BeClass 報名 Excel 的「`可承接的胎數`」欄位，當勾選「雙胞胎」或註記多胞胎時，寫入 `care_babies = 2` (或對應數字)。
...
### 5.4 ⚠️ 建議欄位四：排定放假日期 JSON 欄位 (`custom_rest_dates`)
* **業務需求**：月嫂排班與行事曆出勤天數精算中，行政專員動態選擇/調整之休假日期（每週例休、國定假日休假、請假）需進行資料庫持久化，提供「儲存放假與動態順延」防呆機制，並自動後延完工日 `end_date`。
* **修改建議**：
  * 在 `orders` 資料表新增 `custom_rest_dates` (`JSON NULL COMMENT '排定/自訂休假日期 JSON 陣列 (如 ["2026-07-05", "2026-07-12"])'`)。

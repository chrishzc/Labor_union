...
> **建立時間**：2026-07-22  
> **核心原則**：**絕不動到既有的 `generate_fake_data.py` 與 `假資料_範例.xlsx` 基準檔**。採用「增量擴充腳本 (如 `scripts/seed_boundary_anomalies.py`)」獨立建立邊界案件與異常用例，寫入 DB 提供管理端 UI 與核心系統進行完整的邊界與異常邏輯測試。
...
## 🛠️ 3. 擴充寫入資料庫實作方案 (Incremental Seeder)

為確保完全不動既有的 `generate_fake_data.py` 及 `假資料_範例.xlsx`：

1. **專用編號區隔**：
   - 擴充測試案件 `case_no` 使用專用區段：**`115900001` 至 `115900050`**。
   - 擴充月嫂 `identity_card` 使用專用字號 (如 `A900000001` 起)。
2. **獨立擴充腳本**：
   - 新建 `scripts/seed_boundary_anomalies.py` 腳本。
   - 以增量 `INSERT` 方式將上述 A～G 類別之測試個案直接寫入 `union_db`。

---

## 📌 4. 待辦事項與執行清單 (TODO Action Items)

- [ ] **[TODO-01]** 建立獨立擴充播種腳本 `scripts/seed_boundary_anomalies.py` 框架。
- [ ] **[TODO-02]** 實作【類別 A】格式與型態無效（銀行代碼中文、非法日期、非數字天數）之測試資料寫入。
- [ ] **[TODO-03]** 實作【類別 B & C】BeClass 錯配、身分證重複、月嫂排班撞期與技能不符個案寫入。
- [ ] **[TODO-04]** 實作【類別 D & E】客戶/補助/月嫂金額不符、短撥、溢撥與多帳號共用之金流列與 `pending` 狀態寫入。
- [ ] **[TODO-05]** 實作【類別 F】包含 `CLIENT` / `RETURN` / `SUBSIDY` / `STAFF` 領域及 `claimed` / `resolved` 狀態之警示與事件寫入。
- [ ] **[TODO-06]** 實作【類別 G】LINE 重複綁定與推播失敗請求列寫入。
- [ ] **[TODO-07]** 執行 `seed_boundary_anomalies.py`，並於後台 UI（包含 Page 6 帳務警示中心、排班表、客戶列表）完成全功能邊界測試驗收。

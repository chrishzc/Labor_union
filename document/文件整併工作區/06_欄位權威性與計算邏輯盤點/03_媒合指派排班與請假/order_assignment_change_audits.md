...
- 子表關係：無已宣告子表
- 已確認跨表裁決：本表為不可變的事件稽核日誌 (Immutable Event Log)，負責記錄當「配對方案 (`caregiver_matching_plans`)」正式被套用、寫入 `case_staff_assignments` 時的同步稽核快照。`order_before_snapshot` 已自 schema 與欄位清冊移除，以符合 Lazy 最小化儲存原則。

...
- 已確認跨表裁決：本表為正式訂單服務指派的核心實體。配對方案轉為正式訂單時產生指派；結算前任何條款、分段、請假或代班變更均取消舊有效指派並建立新指派。`actual_hours` 是正式工作日與訂單每日服務時數的衍生投影，不是可人工覆寫的來源事實。
...
| `floor_fee_allocated` | `DECIMAL(12, 2) NOT NULL DEFAULT 0.00` | 本指派承接的樓層費分配投影。 | 衍生投影 | 由 Preview 對有效 assignment 分配 `orders.floor_fee`；所有非 cancelled assignment 的 allocation 合計必須等於 `orders.floor_fee`。 | 訂單樓層費條款與 Preview 分段／代班分配。 | `orders.floor_fee` 與已確認的 assignment segment／代班安排。 | 不具獨立費用權威；上游樓層費、分段或代班變更時，在凍結前取消舊 assignment、以新 Preview 重新分配，且守恆 `Σ active floor_fee_allocated = orders.floor_fee`。代班僅轉移原 assignment 對應比例，不得創造或重複樓層費。 | Assignment Preview／Apply Projection | 樓層費或任一上游條款變更 | 訂單尚未完成或相關帳務尚未核銷／結算時可重建；兩者均完成後差額走 adjustment／reversal | 現況公式空白，容易被誤當可直接填寫的薪資來源。 | 已確認：訂單樓層費守恆分配 |
| `status` | `ENUM(...) NOT NULL DEFAULT 'planned'` | 指派生命週期投影。 | 狀態投影 | 不計算。Preview 是純試算、不建立 DB assignment；Apply 後的有效指派依進度呈現 `planned`、`active` 或 `completed`，取消／重排舊指派一律為 `cancelled`。 | Assignment Apply／Cancellation command 與正式工作日／業務時間投影。 | 已接受的建立／取消命令及 assignment-owned 正式工作日。 | `planned`＝已 Apply、尚未實際服務；`active`＝實際服務期間；`completed`＝本 assignment 分段服務已完成（整案可仍服務中）；三者均為有效指派並套相同薪資公式。`cancelled` 僅作有效性篩選，排除目前排班與薪資投影。`replaced` 不再是業務狀態，長期自 ENUM／caller 移除。 | Assignment Apply／Cancellation／進度 Projection | Apply、取消、正式服務進度變更 | 凍結前仍可取消／重建任一有效狀態 assignment；訂單完成且相關帳務核銷／結算後差額走 adjustment／reversal | 現況仍有 `replaced` enum／caller，會讓有效性判斷分叉。 | 已確認：完整 assignment 狀態定義 |

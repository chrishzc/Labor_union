...
- 數量：18
- 本文件只完成 path／type／bytes／SHA-256 與架構分類，尚未宣稱完成附件內容語意審查。
- `human-content-review-required` 附件在人工確認內容已被正式規格覆蓋以前，
  仍是本次整體架構確認的 evidence gate。

## 分類

| 路徑 | Bytes | SHA-256 | 分類／處置 |
|---|---:|---|---|
| `document/LINE (1).pptx` | 2650667 | `2f1fbc7a15a784bcfcc4d2dc8b33817e854b8de0b85cf07a57d6983b2c9b683f` | `human-content-review-required`；LINE／BreezySign／Knowledge 產品來源 |
| `document/月子媒合流程圖.canvas` | 11834 | `574de2454b94eb85785d36741cf9f0791703912402496133e4f38ae1a258c7c5` | `human-content-review-required`；跨 Domain 流程視覺來源 |
...
| `document/資料庫、資料處理/帳務.xlsx` | 8153 | `bd22f7459a8755cba2e7347d8a51f08ef01420534d39b057b04d6317b5967788` | `human-content-review-required`；Client Finance／Staff Payables 欄位來源 |
| `document/資料庫、資料處理/資料庫來源表.xlsx` | 19944 | `2863081b7773900cd01116aedc0fcffe0bfa6cd12b0eca5d3d43964462c7fe4c` | `human-content-review-required`；Case Import／欄位 lineage |
...
| `document/管理端UI/系統異常警示中心規格書.docx` | 33216 | `2bbfe70babb46a66e6242fdb49fce10a74c6da5748b093b03573aed44c25e825` | `human-content-review-required`；Anomalies 產品來源 |
...
| `document/管理端UI/表格需求模板/服務人員契約.xlsx` | 371317 | `247a0e34644ca3551cea393498c349102714bf5604a9a3d5b511929392e3788c` | `human-content-review-required`；Contract Integration／Staff 產品來源 |
...
| `document/管理端UI/表格需求模板/週報.xlsx` | 425220 | `0ec9706931a09b2e5a8f20572972fe8d9103008f6aeed0c4ee2b8d1f99880013` | `human-content-review-required`；Reporting Query 邊界 |
...
| `document/管理端UI/資料庫原始資料瀏覽_頁面欄位開放權限建議表.xlsx` | 15019 | `a9a9db6d981ae5aa6de3f269c4eb16f6054769afe374a2e2cff089fff600f524` | `human-content-review-required`；Access Control／Data Browser 來源 |
...
3. `human-content-review-required` 必須逐頁／逐 worksheet 產生內容摘要、來源對照與
   `covered | conflict | historical | out-of-scope` 結果。
4. hash 改變時，本索引失效；不得沿用舊的內容審查結論。
5. 附件內容審查不得直接修改 production code 或把視覺欄位推定成 DB SSOT。

## 人工確認 Gate

`15`～`18` 可以作為 Markdown 來源的完整候選架構，但整個 `document/` 的來源覆蓋
只有在本表所有 `human-content-review-required` 完成內容審查後才可宣稱完成。

...
#### 3. 細節測試內容與正則驗證
- **UI 彈窗與導流檢查**：
  - 點選「未申請市府平台」：跳出引導彈窗，顯示說明並提供外連市府月子平台登記之按鈕；彈窗註明登記完成後請回 LINE 填寫問卷。
  - 點選「已申請市府平台」：展開身分核對輸入區塊（姓名與手機號碼）。
- **輸入欄位正則驗證 (Regex)**：
  - **手機號碼**：正則式 `^09\d{8}$`。輸入 `091234567`（9碼）、`0800000123` 或包含英文字母時，欄位下方需即時顯示紅字錯誤提示「請輸入正確的 10 碼台灣手機號碼（09 開頭）」，且送出按鈕維持禁用。
  - **姓名**：長度需介於 2~20 字元，不可為空，自動去除前後空白字元（Trim）。
- **資料庫查核**：
  - 僅執行 `clients` 表的讀取比對，**不可**對任何表格產生未授權之寫入。

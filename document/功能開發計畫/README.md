...
| 文件 | 狀態 | 下一個 gate |
|---|---|---|
| [Current-state 異常機制瘦身完整執行計劃](PROV-20260829-current-state-anomaly-slimming-execution-plan.md)（[post-prep amendment](PROV-20260829-current-state-anomaly-slimming-post-prep-amendment.md)；[Task 97 Authority reconciliation](PROV-20260830-current-state-anomaly-task97-authority-reconciliation.md)） | `APPROVED_ADDITIVE_SUCCESSOR / STABILIZATION_REQUIRED`；destructive cutover `NOT_AUTHORIZED` | 以 Task 97 current stabilization為先；不得重開 current-only產品方向，也不得新增 retirement/schema。先修 current CI並重綁 current-head evidence；之後只處理既有 writer exits、runtime detector/lock、1016 DB engine、external/public cutover與zero-reference gate。舊 99-path denominator與 `read-only only` Authority不得再作 current execution truth。 |

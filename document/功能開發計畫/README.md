...
| 文件 | 狀態 | 下一個 gate |
|---|---|---|
| [Current-state 異常機制瘦身完整執行計劃](PROV-20260829-current-state-anomaly-slimming-execution-plan.md)（[Task 97 Authority reconciliation](PROV-20260830-current-state-anomaly-task97-authority-reconciliation.md)；[parallel execution SSOT](PROV-20260830-current-state-anomaly-parallel-execution-refresh.md)） | `ANOMALIES_REPOSITORY_LOCAL_PARTIAL / integrated in PR #63`；destructive cutover `NOT_AUTHORIZED` | 15-code current-only identity/API/UI與Task 97 shared governance已整合；`LINE-004` typed consumer已接通。`LINE-006`及另外13個owner contracts仍為`DEFERRED_OWNER_CONTRACT`，後續依owner另拆包。 |
| [LINE 後端瘦身執行計畫](LINE_BACKEND_SLIMMING_PLAN.md)（[historical/current-head pre-refresh amendment](LINE_BACKEND_SLIMMING_POST_PREP_AMENDMENT.md)；[parallel execution SSOT](LINE_BACKEND_SLIMMING_PARALLEL_EXECUTION_REFRESH.md)） | `LINE_BACKEND_SLIMMING_REPOSITORY_LOCAL_PARTIAL / integrated in PR #63` | direct cross-domain writer、legacy delivery caller、provider bypass與legacy role authority已完成repository-local收斂；M1 schema雙角色、M3 candidate delivery與M4 `LINE-006`正式契約仍deferred。 |

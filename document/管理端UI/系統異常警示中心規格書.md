...
### 1.3 異常中心作為正式保護機制（2026-08-02 人工確認）

異常處理不是流程末端的補救頁面，而是整體架構的第二層保護。所有 Domain 統一採下列三層：

1. **上游根事實寫入防護**：建立或修改根事實時驗證必要欄位、型別、ownership 與合法狀態；不允許寫入明知不完整的正式事實。
2. **異常中心提前偵測**：持續由根事實與正式事件推導異常，在形成排班、應付款、匯出或結案之前提醒人員補正。修正根事實後由 projector 自動解除；人工 resolve 只管理處理進度，不能取代根事實修正。
3. **下游最低限度防禦**：下游不為每個可提前發現的缺漏再建立一套業務分支，只驗證版本、冪等、併發、金額精確打平、唯一 ownership 與交易原子性等硬不變量。異常資料若仍流到下游，一律 fail closed。

共同規則：

- 警示只能從可追溯的根事實、正式事件或一致性檢查推導；UI 不得直接建立警示，也不得直接修改衍生 `amount_delta`、付款狀態、訂單狀態或排班結果來消除警示。
- `claimed／resolved` 是人員工作流，不是 Domain facts。根條件仍存在時，current-state 警示在下一次投影必須 reopen。
- `finance_alerts` 保存需要不可變稽核的 occurrence；`system_alerts` 保存目前仍存在的流程／資料異常投影。兩者可以指向同一根事實，但不得互相冒充正式帳務或狀態機。
- 不把正常流程中不可能成立、或已由上游不變量阻擋的假情境設計成額外關卡。只有 legacy 髒資料、寫入邊界被繞過或真實世界資料需要人工確認時才產生異常。
- 非正常時序（例如取消或服務日更正後才在原應付日之後形成月嫂差額）不自動改期或塞入下一個正常付款週期；直接進異常中心由人員處理。
...
| 前綴 | 領域 | 代碼數 | 對應節次 |
|---|---|---|---|
| `RECEIVABLE-` `PAYOUT-` | 帳務時效性、付款主檔完整性及非正常付款時序提醒（與 3.1 的核銷代碼互補） | 4 | 3.2 |
...
| | `staff_monthly_settlement_ambiguity` | Legacy 月結單不唯一／歧義 | 既有 production caller 仍遇到多筆待撥款月結單或跨月候選歧義 | `pending` / 建立 `STAFF` 警示 | ✅ 既有相容路徑已接上；2026-08-02 已確認新架構不建立月結狀態機，caller 遷移前保留此警示 |
...
| `PAYOUT-002` | 原應付日後才形成或改變月嫂應付義務 | 訂單取消、實際服務日更正、assignment 重建或 adjustment 在原 `orders.staff_payment_due_date` 已過後，使有效應付金額新增或改變 | 高 | 保留原訂單應付日，不自動延到下一個 15 日；列出根事件、原應付日、變更前後金額與受影響 assignments，交人員決定後續付款／調整 |
| `PAYOUT-003` | 月嫂付款必要主檔不完整 | 已存在有效應付來源，但月嫂沒有有效且唯一的主要銀行帳戶，或必要帳戶資料不完整 | 高 | 應在正常匯出前提早顯示並要求補正根主檔；匯出層若仍收到此資料視為一致性錯誤，不提供略過或人工挑選分支 |
...
| `IMPORT-006` | 銀行匯入批次完整性異常 | canonical row 解析缺列、fingerprint collision、occurrence 缺失、批次部分完成，或 classification／reconciliation 狀態矛盾 | 高 | 既有批次投影已通過本機 2,659／2,655 隔離真實 MySQL Subsystem 驗收；但 live 實作仍把普通 `non_business_review` 彙總於此，須依 2026-08-02 裁決遷移為完整性專用。ASUS 真實主機驗收暫緩 |

**帳務待確認與 `IMPORT-006` 的分界（2026-08-02 已確認目標）：**

- 普通 `non_business_review` 或可安全保存但暫時判不出業務歸屬的列，逐 canonical row
  建立 `finance_import_manual_review`，顯示於「帳務」分頁；不再彙總成 `IMPORT-006`。
- 財務警示 identity 為 `finance-import-row:<finance_import_row_id>`，同一 canonical bank fact
  同時最多一筆 active review，儲存在 immutable／event-sourced `finance_alerts` 與
  `finance_alert_events`。
- 人員可在帳務區選擇正確帳務類型與關聯義務，先看修正影響 Preview，再按
  「確認修正並入帳」。UI 只送 typed command，不得直接 SQL 或自行計算。
- 後端以單一 transaction 寫入 classification event、正式 ledger／allocation、
  reconciliation receipt 與 alert resolved event；任一步驟失敗全部 rollback。
- 銀行日期、金額、方向、帳號、撤銷碼、raw payload、fingerprint 與 occurrence 不可由
  修正介面改動。金額未完整 allocation 或任一所選義務未精確歸零時不得入帳，警示維持。
- `IMPORT-006` 只保存匯入完整性異常的批次摘要；`case_key` 固定為
  `finance-import-batch:<batch_id>`，每個批次最多一筆，並阻擋該批正式 Apply。
- details 只保存 occurrence／distinct／inconsistent counts、原因分布、最多 20 個 sample row
  ids 及最近重處理摘要；不得保存姓名、完整帳號或 raw payload。
- 初次匯入完成、歷史批次 scan 或重處理完成時刷新；一般頁面載入只讀已物化警示，
  不得每次重算銀行流水。
- 普通待確認與完整性問題不得因同一根因重複出現在兩區；完整性故障優先阻擋，修復後才
  投影可操作的帳務待確認。
...
2. 已完成的既有切片包含 ORDER-001~004、BECLASS-001、LINE-001/005、DOC-SEND-001、IMPORT-001~006、RECEIVABLE-001、PAYOUT-001、RETURN-001。舊版 IMPORT-006 批次 current-state 投影、API／UI 接線與本機 Subsystem 驗收已完成；2026-08-02 新裁決要求把它收斂成匯入完整性異常，並新增逐列 `finance_import_manual_review` 修正／入帳工作台，此目標架構尚未實作，不得用舊驗收宣稱完成。

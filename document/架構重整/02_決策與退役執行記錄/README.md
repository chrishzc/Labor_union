...
|---|---|---|---|
| [匯入入口與 Legacy Writer 退役](Import_Entry_and_Legacy_Writer_Retirement_工作包.md) | work-package | `blocked` | 只剩 Client／Staff verified-LIFF replacement acceptance 與 dead historical SQL/helper audit；current profile mutation contract 依 `20` §6.1，entry retirement 依 `19`。 |
...
- [歷史案件作業基準](PROV-20260827-historical-order-operational-baseline-spec.md)：current Orders baseline／storage補充仍有 formal／gap consumer；舊 package execution snapshot 已由 Git history 保存；
- [歷史付款與owner結清規格](PROV-20260828-historical-payment-and-owner-settlement-spec.md)：current Client Finance Arch Map／formal owner仍直接引用；舊 package execution snapshot 已由 Git history 保存；
...
這些文件的current consumer只限既有正式契約、navigation與validation identity；舊execution ledger、package readiness或未完成snapshot不得覆蓋current formal spec、current source／tests／readback。
...
M1 binding ownership、M3 matching coordination、M4 human escalation 的 backend closeout 已封存；其尚未完成的 verified LIFF、provider、UI 或資料情境只能依 current `17`／`20`／`19` 與各 owning Domain successor 恢復，不能因舊包封存而宣稱通過，也不能重開舊 Work Package。

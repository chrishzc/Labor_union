...
- **Active／residual Task 96 IDs：** 仍有 `in-progress`、`partial`、`local accepted/pending GitHub Actions` 與 `deferred/not_run` lanes；不得以文件狀態把未完成工作移出 current register。
- **CI candidate：** 舊 source-bearing baseline commit `37c9d063` 對應 GitHub Actions run
  [`33460244467`](https://github.com/chrishzc/Labor_union/actions/runs/33460244467) 只作歷史 baseline evidence。current local
  React／launcher evidence 已由 root accepted；current commit 尚未產生 GitHub Actions run，暫標 local accepted／pending GitHub Actions。
...
| Code-bearing baseline CI | `local accepted/pending GitHub Actions` | source：current local React 184 files／1195 tests、`npm build`、Clients＋launcher focused 8 passed；runtime：local acceptance passed，舊 `37c9d063`／Actions run [`33460244467`](https://github.com/chrishzc/Labor_union/actions/runs/33460244467) 只作 baseline provenance；terminal：current commit 尚未產生 GitHub Actions run，不能寫成 current CI PASS。 |
...
| `CUR-CI-CODE-BASELINE-ACTIONS-01` | `local accepted/pending GitHub Actions` | source：current local React 184 files／1195 tests、`npm build`、Clients＋launcher focused 8 passed；runtime：local acceptance passed，`37c9d063` 舊 Actions run僅為 baseline；terminal：current commit尚未產生 GitHub Actions run，舊 run不代表current HEAD或full Task 96 acceptance。 |
...
| `CUR-P0-HISTORICAL-PAYMENT-SETTLEMENT-01` | `partial` | source：Staff Payables integration test與既有 owner package；runtime：root rerun 1 passed，`lu_test_task96_historical_staff_r2` real-MySQL lifecycle passed；terminal：Browser mutation `not_run`，Client owner MySQL仍未驗，React／local CI已 accepted但current GitHub Actions pending。 |
...
- 舊 code-bearing baseline CI 只作 provenance；local React／launcher 已 accepted，current GitHub Actions pending。不得解除
...
- 本 lane 只更新本 register；不宣稱 Task 96 complete，不把 pending GitHub Actions evidence 寫成 passed。

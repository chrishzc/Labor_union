...
| CUR-P0-HISTORICAL-STATUS-012-01 | P0 | `completed` | Orders／`01`、`17` | 六欄 historical order source status 必須精確判定 `0→取消`、`1→已付訂金（Orders訂單成立／Step 5）`、`2→洽談中`；numeric `0` 不得與空白共用 fingerprint，Preview／receipt／React 必須顯示守恆 counts。不得擴張成 generic status editor 或 schema 變更。 | 既有六欄 parser／Preview／Apply閉環保留；2026-08-31依最新人工裁決修正status `1`語意並完成focused Python、React與build複驗。真MySQL與Browser須在合法runtime另驗；final receipt仍保留為先前版本證據，不得外推舊的`1→完成`語意。 |
...
| CUR-LOCAL-DB-1003-CURRENT-01 | S1 | `blocked` / `BLOCKED_DB_TEST_ENV` | Global Migration／`10`、`18`；`PROV-20260828-local-db-1003-to-current-upgrade-spec.md` | exact 1003=`matching_coordination_successor`；current repository靜態chain已延伸至1020，8/28規格的1012只是historical snapshot。既有Static release、Descriptor、read-only plan與舊Engine evidence保留，但1013～1020不得無新鮮engine evidence就外推為qualified。 | 2026-08-30 Stage 0主代理與獨立Luna High複驗均確認：current host已有project `.venv`，但缺少`.env`／DB target設定、MySQL client／Docker capability與可驗證`lu_test_*` identity，因此ordered upgrade、resume、preserve-data readback與normal startup均`NOT_RUN`，總結`DB_CHANGE_NOT_READY`。此blocker不阻擋repository-local owner contract inventory，但阻擋current schema／runtime completion；不得改用SQLite、mock、`union_db`或production假裝通過。 |
...
| CUR-LIFF-PROFILE-01 | P0 | `approved`／`CLIENT_CONTRACT_READY`／`BOUNDARY_REQUIRED_PUBLIC_API`／`DB_CHANGE_NOT_READY` | Client／Staff／Scheduling owner／LINE intake；`20` §6.1、`23`、`24`；`PROV-20260827-liff-profile-change-spec.md` | 最新人工裁決固定第一階段只做Client，exact allowlist為`name`、`gender`、`phone`、`city`、`address`、`residence_type`、`delivery_type`、`baby_info`、`notes`；closed validation、root=`client_id`、專用monotonic `client_profile_version`、requested-field before-values fingerprint、完整本人／授權reviewer顯示及Client-owned request／approval contract均已收斂。Scope與change inventory為`PASS`，不含Staff、seed、business-row backfill、destructive或LINE-owned Client writer。 | Current沒有profile applicant／reviewer Query／Preview／Apply／readback API；依最新邊界控制，新增public entry point須人工明確核准，因此production/schema施工停在此邊界，未旁路放入`line_bot.py`或既有identity endpoint。最小必要裁決僅為核准在既有Client／LINE composition新增bounded typed applicant及internal reviewer endpoints；Staff仍後續獨立package。 |
...
| CUR-ANOMALY-OWNER-BACKEND-PREREQUISITES-01 | S3 | `in-progress` / `OWNER_MATRIX_8_REPOSITORY_LOCAL_PASS` / `BOUNDARY_REQUIRED_CASE_PAIRING_LINEAGE` / `AUTHORITY_REQUIRED_5` | Scheduling／Government Subsidy／Staff Payables／Case Import／Finance Import；`02`、`05`、`06`、`09`、`14`、`17`、`22` | `SCHEDULE-002/003/006`、`GOVSUB-001/002/004`、`BECLASS-001`、`IMPORT-003`已建立owner-typed zero-write readback、closed reasons、Anomalies typed consumer與既有outer UoW內bounded recheck composition；沒有新schema／public API／generic mutation service。Scheduling replacement／overlap／coverage、Government receipt／manual allocation／reversal／overpayment disposition及Case exact `bound_case_no`／same-source accepted receipt均有focused oracle；owner packages＋Task97 affected tests `109 passed`，formal baseline與`git diff --check` passed。Arch Map scoped新增leaf無錯；全域validator仍只回報既有26組duplicate-root inventory。 | Case Import synthetic counterpart與same-source accepted receipt可terminal；但「原identity-conflict review → 後來不同source identity的新accepted source」目前沒有immutable exact lineage，且privacy-safe review root不保存可供推導的case／query identity。補此關聯需新增／修改owner persistence或public Apply contract，超出本包effect ceiling，故`BOUNDARY_REQUIRED_CASE_PAIRING_LINEAGE`。另`PAYOUT-002`、`GOVSUB-003/005/007`、`IMPORT-006`仍因合法correction／disposition operation非唯一而保持`AUTHORITY_REQUIRED`。無configured `lu_test_*`，MySQL engine驗收`not_run`；Stage 4不得把repository-local證據外推為runtime terminal。 |
...
33 個 active anomaly。v1 只有 status `0/1/2=取消／已付訂金／洽談中`，沒有獨立「服務中」欄位；直接合法

...
4. **Anomalies closure**：最後才消費前三階段完成的owner contracts，補detector／projection／action descriptor；
   owner mutation與durable recheck intent必須同一outer UoW提交，worker再讀fresh owner facts。完整9-code matrix
...
| 4 | Anomalies closure | `CUR-P0-ANOMALY-RECOVERY-01` | 只組合已完成owner contracts；補detector／projection／descriptor、mutation後fresh recheck與完整9-code terminal matrix。 |
...
| CUR-P0-ANOMALY-RECOVERY-01 | S4 | `completed` / `NINE_CODE_CURRENT_CLOSURE_LOCAL_PASS` | Anomalies消費端＋各owner；`06`及9-code source map | current registry只保留9個具真實營運情境的issue；Anomalies只組合owner Query／Preview／Apply／receipt／readback，不新增generic owner writer、claim／resolve替代路徑或直接SQL。 | 9碼均具source-bound action、owner mutation同UoW recheck與fresh predicate readback；未授權production／deployment／entry switch。 |
...
| CUR-LOCAL-DB-1003-CURRENT-01 | S1 | `completed` | Global Migration／`10`、`18`；`PROV-20260828-local-db-1003-to-current-upgrade-spec.md` | exact 1003=`matching_coordination_successor`；current repository靜態chain已延伸至1021，8/28規格的1012只是historical snapshot。1021 static release、descriptor、fresh assembly與affected terminal baseline已形成；既有舊Engine evidence保留，但1013～1021不得無新鮮engine evidence就外推為qualified。 | Current host已在Docker `mysql_db`內完成明確非`union_db`／production的`lu_test_task96_ldu_source_1003_r1`官方preserve-data same-name replacement。r1曾揭露1021 FK型別錯誤，依人工裁決不新增1022，直接修正尚未qualified的1021；r2 preserve candidate與獨立fresh bootstrap均verified。official r4流程完成source backup、candidate restore、1004～1021 sequential apply、candidate verify、candidate dump、same-name replacement與post-replacement data／schema-program／owned-object verify；`--require-current`回報current latest=1021。人工核准後精確終止佔用8000的舊PID 28964，normal no-auth launcher重跑時DB current gate、FastAPI／React ready、Runtime Monitor／Durable Background Worker／Incident Maintenance Worker連續200均通過；LINE credentials缺失依規格skip。Browser GET的`/admin/`具`#root`、顯示「系統在線」且console error=0，laucher log證明同源`/api/v1/system/status/performance-snapshot`、orders summaries與operational timelines均200。驗收後只停止本次owned runtime，8000／5173均已釋放。LDU-A1～A11及Developer acceptance收旂。 |
...
| CUR-LOCAL-DB-PORTABILITY | S1 | `completed` | Global Migration／`10` §4.5 | 已移除 `lu_test_*` 名稱綁定與 shared qualification 的 reference DB／host／port／資料指紋耦合；每台機器 Apply 前仍必須建立release-scoped local dump／receipt，DDL前後驗row evidence，journal存在但原備份遺失時fail closed。 | current host的exact test target、container、zero-write replacement Preview、source backup、candidate restore、1004～1021 engine apply、verified preserve-data candidate、canonical fresh bootstrap、official same-name replacement、replacement readback、normal no-auth runtime、required workers與Browser GET均已通過；source current latest=1021，owned runtime已scoped cleanup。remote／production／`union_db`／MySQL system schema維持封鎖。Stage 1 runtime terminal gate完成，可依register priority交接Stage 2。 |
...
2026-08-31 `CUR-LINE-MODULES-1-4-CLOSURE-01` current supersession：本段取代主列的
`in-progress`／`DB_CHANGE_NOT_READY` 與舊環境阻擋描述；current status=`completed`。M1 已在一次性
`lu_test_*` source／candidate 以 1003 代表性 legacy binding／event 資料完成 canonical 1004→1021
preserve-data migration；1019 role-scoped binding／event successor exact、legacy rows immutable、source
unchanged、candidate 1021 exact，Gate 5／6／7 均 `PASS`，且測試資料庫已精確清理。M2～M4與
LINE-006沿用本列及下方final evidence，不重跑或重寫已完成coverage。provider實送、verified-token
applicant、Rich Menu、Scheduling mobile public surface與deferred項目仍依各自lane，不是Stage 2 terminal
條件。Stage 2完成，依current priority進入Stage 3 owner backend matrix。

2026-08-31 `CUR-ANOMALY-OWNER-BACKEND-PREREQUISITES-01` current supersession：本段取代主列的
`in-progress`／`ENVIRONMENT_BLOCKED` 與舊DB環境描述；current status=`completed`。Case Import、
Payroll／Staff Payables、Finance Import與Government Subsidy的裁決後owner contracts、concrete repositories、
fresh readbacks及focused evidence沿用主列與current workspace。官方1021同名replacement已在既有
`lu_test_*` preserve-data candidate以MySQL 8.0完成canonical 1003→1021 ordered apply與exact verify：
release／descriptor一致、1021 tables／constraints／immutable triggers可由真engine建立、既有資料preserved、
view mismatches=0；這解除原本僅因無可解析test target形成的environment blocker。依人工裁決不另外要求
第二個「合法DB」分類，也不建立1022。Stage 3完成，Stage 4可開始消費既有owner contracts。

2026-08-31 `CUR-P0-ANOMALY-RECOVERY-01` pre-action baseline：status=`in-progress` /
`FIFTEEN_CODE_CONSUMER_COMPOSITION_LOCAL_PASS` / `PRE_ACTION_BASELINE`。Anomalies runtime現已以
typed、zero-write owner snapshots組合15碼current predicate：`LINE-004/006`、`SCHEDULE-002/003/006`、
`GOVSUB-001/002/003/004/005/007`、`BECLASS-001`、`IMPORT-003/006`及`PAYOUT-002`；Anomalies不查owner
private tables、不重算owner business facts，也不新增replacement writer。此次漂移修復補齊Payroll、Finance
Import與Government Subsidy 003／005／007的runtime detector composition及canonical tests。Stage 4仍未
terminal；affected focused matrix=`43 passed`。15碼action descriptor／authenticated owner entry可達性尚未
逐碼證明，且各owner mutation與
`anomaly.recheck` intent同一outer UoW、worker fresh readback及Apply後合法刪除仍須完成matrix evidence；
`LINE-004`現有owner operation是否足以處理same-type duplicate root亦未由current contract唯一證明。依
2026-08-31人工指示，本slice完成focused驗證與文件／Arch Map reconcile後暫停，不開始下一個施工方向，
不commit、不push。Final static evidence：changed Python compile、strict UTF-8及`git diff --check`均passed；
scoped Arch Map validator已無本slice layout gap，只回報25個既存`DUPLICATE_TEST_ROOT_OWNER`，依人工指示
不在本slice處理。

2026-08-31 `CUR-P0-ANOMALY-RECOVERY-01` current supersession：current status=`in-progress` /
`EIGHT_CODE_BOUND_ACTION_LOCAL_PASS` / `EIGHT_CODE_OWNER_RECHECK_LOCAL_PASS` / `BOUNDARY_REQUIRED`。
8碼已將current issue綁定到既有owner
operation，並在detail回傳JSON-safe、source-bound manual action：`SCHEDULE-002/003/006`、
`GOVSUB-001/002/004`、`BECLASS-001`、`LINE-006`。對應focused evidence為
Scheduling `39 passed`、Government Subsidy `37 passed`、Case Import `61 passed`、LINE `51 passed`；
原15碼current predicate matrix `43 passed`保留。這些action不新增owner writer、public API、schema、
UoW或provider effect。

剩餘7碼已完成public reachability核對，不得以靜態operation名稱冒充terminal-ready action：

- `PAYOUT-002`已有Payroll-owned Query／Preview／Apply／receipt／fresh readback，但沒有authenticated
  public entry；若新增為public API，依current boundary必須先取得最小entry裁決。
- `GOVSUB-003/005/007`已有Government Subsidy owner application contracts與concrete repository，但
  current `government-subsidy` routes沒有三碼專屬authenticated recovery entry；不可把
  query-only batch、normal return reconciliation或repository method冒充。
- `IMPORT-006`已有historical reprocess與corrected-source ingest路徑，但前者不是本碼的
  closed deterministic rebuild contract，後者也沒有完整Query／Preview／Apply recovery entry；
  需先固定兩分支最小public composition。
- `IMPORT-003`的正式owner operation是新HCM source的`IngestCaseImportSource`→
  `Preview／ApplyCaseImport`，不是field-level `HcmResubmission`。current HCM workbook Apply尚無
  `original_review_identity`、exact accepted lineage與pairing recheck的public composition，不得掛錯入口。
- `LINE-004`雖有identity replacement Preview／Apply，但current contract未證明它能處理
  same-type duplicate root；直接掛接會改owner semantics。

因此Stage 4仍未terminal；上述7碼分類為最小`BOUNDARY_REQUIRED`，不因此回開已完成的
Stage 2／3或8碼slice。
8碼slice已證明owner mutation與`anomaly.recheck` intent共用outer UoW、worker組合fresh typed
owner readback，且generic reconcile只在authoritative complete的predicate absence刪除issue。final combined focused
`146 passed`；changed Python compile、strict UTF-8與`git diff --check` passed。Arch Map validator對本slice
已無source-owner或test-layout gap，只剩25個既存`DUPLICATE_TEST_ROOT_OWNER`，依人工指示不處理。
本slice不commit、不push。

2026-08-31 `CUR-P0-ANOMALY-RECOVERY-01` latest business-scenario supersession：current
status=`completed` / `NINE_CODE_CURRENT_CLOSURE_LOCAL_PASS`。最新人工裁決確認
`PAYOUT-002`、`GOVSUB-003/005/007`、`IMPORT-003/006`不是現行營運異常，不得以低機率或假設情境
建立修復框架；六碼已退出current registry、新事件投影、定期掃描、recheck composition與UI target，
且不轉成generic owner work item。薪資由正式服務天數等Payroll owner facts計算；政府補助季別加總與
Finance Import整理公式改由發布前deterministic tests防止；客戶個資修正沿用Client owner既有流程。
已發布1021保持immutable，不新增1022，也不刪既有additive tables；未再使用的結構列為技術債。

現行9碼為`SCHEDULE-002/003/006`、`GOVSUB-001/002/004`、`BECLASS-001`、`LINE-006`、
`LINE-004`。LINE-004沿用既有same-type identity replacement：新客戶案件可取代舊customer binding，
即使新案件已保存相同LINE ID亦合法；只清除舊customer projection，staff role保持不變。替換在既有
outer UoW追加bounded LINE-004 recheck，Anomalies detail只source-bind既有Preview／Apply，不寫LINE root。
既有已建立delivery不追改，後續delivery讀新binding。本段supersede上方15碼／剩餘7碼
`BOUNDARY_REQUIRED`描述；不重開Stage 2／3、不修改解除saga、Rich Menu provider邊界或LINE-006語意。
...
2026-08-31 DB environment classification addendum（current supersession）：current host現已從Docker
`mysql_db`唯一解析`lu_test_task96_ldu_source_1003_r1`，且zero-write source readback證明1003 exact、official
replacement Preview列出1004～1021，因此舊`ENVIRONMENT_BLOCKED`不再成立。current blocker是實際
replacement／same-name replacement未在既有Authority內，分類為`BOUNDARY_REQUIRED_REPLACEMENT`；不得改用
替代schema、DB path、fixture PASS或production／`union_db`繞過。

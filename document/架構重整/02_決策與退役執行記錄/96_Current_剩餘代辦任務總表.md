...
| CUR-LINE-MODULES-1-4-CLOSURE-01 | S2 | `blocked` / `OWNER_CONTRACT_PARTIAL` / `BOUNDARY_REQUIRED` | LINE／Access／相關owner；`17`、`20`、`23`、`26_LINE四大模組Eraser流程圖轉錄與驗收基線.md`；`PROV-20260828-line-modules-1-4-closure-spec-gap.md` | 2026-08-30唯讀owner inventory與獨立裁決確認：M1既有typed customer／staff／admin binding、review、owner projection、aggregate readback、revocation及同型replacement route／application；但目前單列identity root不能持久化合法customer+staff雙角色，缺role context／Rich Menu switch-restore、staff retirement接線與binding failure→既有durable Customer Service escalation整合。同型replacement repository guard已由`36ac601`移除矛盾的changed-reference拒絕，保留bound／version／same-type／unchanged guards。M3／M4已有recipient／snapshot／token／delivery及generic terminal/config骨架，但candidate contact尚未收斂到canonical delivery lineage，`LINE-006`缺LINE-owned typed current Query／root key／reason／terminal predicate／fresh readback。 | 同型replacement regression已放入新`line-identity-management` Module canonical root；正向更新／version＋1／單一rebound event、unchanged及cross-type三項oracle，加既有repository tests共`25 passed`，fresh Luna High verifier通過。Arch Map本slice已無undeclared source／broad test-root錯誤；全域validator仍因25個既有且無關的duplicate test-root owners為`failed`，本slice不擴張清理。雙角色持久化仍需schema／migration並標`BOUNDARY_REQUIRED`；下一個material gate是另行裁決M1 schema、role context／menu／retirement／escalation與`LINE-006` owner semantics，不得外推provider／LIFF E2E通過。 |
| CUR-ANOMALY-OWNER-BACKEND-PREREQUISITES-01 | S3 | `blocked` / `SPEC_GAP` / `BOUNDARY_REQUIRED` | Scheduling／Government Subsidy／Staff Payables／Case Import／Finance Import；`01`、`02`、`05`、`06`、`09`、`14`、`16`、`17`、`22`、`24` | 2026-08-30五條獨立唯讀owner inventory確認13碼均未達terminal owner contract：Scheduling三碼缺逐碼universe／predicate／root-version-lock／QPA-readback-recheck；Government Subsidy六碼只有generic claim／ledger／overpayment與current-issue primitives，缺逐碼terminal QPA組合；`PAYOUT-002`的late-delta disposition／completion predicate未裁決；`IMPORT-003`、`IMPORT-006`、`BECLASS-001`各自缺accepted-mapping／batch integrity的owner QPA、lock、receipt、readback與recheck。Finance Import、Case Import、BeClass不得合併成generic mutation service。 | 這些缺口會改owner contract、SSOT／root fact或跨Subsystem語意，依後續邊界控制全部標`BOUNDARY_REQUIRED`；主代理未設計、未修改production source。下一個material gate是各owner人工裁決最小predicate、合法operation、version／lock、receipt／fresh readback與recheck intent。Stage 2 `LINE-006` gate及13碼owner contract皆terminal後，Anomalies才可進Stage 4 consumer closure。 |

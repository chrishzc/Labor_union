...
| CUR-ANOMALY-OWNER-BACKEND-PREREQUISITES-01 | S3 | `in-progress` / `OWNER_MATRIX_REPOSITORY_LOCAL_PARTIAL` / `BOUNDARY_REQUIRED_GOVSUB007_ATOMIC_EXCESS_UOW` / `DB_CHANGE_NOT_READY` | Scheduling／Government Subsidy／Payroll／Staff Payables／Case Import／Finance Import；`02`、`03`、`05`、`06`、`09`、`14`、`17`、`22` | 原8碼owner typed readback／consumer／recheck成果保留。2026-08-31後續裁決已新增：Case Import different-source exact accepted lineage；PAYOUT-002 Payroll disposition與必要Staff recovery；IMPORT-006 deterministic rebuild／accepted correction successor；GOVSUB-003 deterministic/typed/structural三分支、GOVSUB-005 versioned correction lineage與GOVSUB-007 recovery readback/reconciliation。1021保存各owner最小additive persistence；無generic remediation／compensation或Anomalies direct writer。 | Case、PAYOUT-002、IMPORT-006、GOVSUB-003/005及GOVSUB-007既有recovery readback/reconciliation已repository-local通過。唯一已知architecture blocker是GOVSUB-007建立：同一actual outgoing須在Government Subsidy單一outer UoW同時核銷lawful return並建立excess recovery；既有workflow自行commit且拒絕overage，未經人工核准不得改transaction contract或拆成兩次commit。MySQL engine/preserve-data仍`not_run`；Stage 4不得外推為runtime terminal。 |
...
但LINE LIFF WebView目前沒有既有機制取得另一個後台tab的Bearer Session；統一storage key只支援同一browser
context，不等於正式mobile session bootstrap。不得在LIFF內複製登入/MFA、由LINE binding自動簽發admin Session，
或降級成LINE-only admin authorization；此正向runtime維持
`BOUNDARY_REQUIRED_MOBILE_PERSISTED_SESSION_BOOTSTRAP`。Feedback與semantic tool router已在`20`

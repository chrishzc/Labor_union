...
|---|---|---|---|
| CUR-LOCAL-START-REACT-01 | `completed` | Windows／Unix 標準本機 launcher、唯讀 preflight 與 GET-only smoke 已收斂為 FastAPI `8000`＋React/Vite `5173`，不再啟動 Streamlit 或等待 8501。focused launcher tests `25 passed`、blocking flake8 `0`；Windows dry-run ready，實際 smoke 僅建立 `api`／`react`，`/admin/` ready、relative `/api` 200，結束後 8000／5173 owned process 均清理。Legacy Streamlit source 未在本包刪除。 | `19` §6；`scripts/launchers/README.md` |

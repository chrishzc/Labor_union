...
| CUR-LINE-MODULES-1-4-CLOSURE-01 | S2 | `blocked` / `OWNER_CONTRACT_PARTIAL` / `BOUNDARY_REQUIRED` | LINE／Access／相關owner；`17`、`20`、`23`、`26_LINE四大模組Eraser流程圖轉錄與驗收基線.md`；`PROV-20260828-line-modules-1-4-closure-spec-gap.md` | 2026-08-30唯讀owner inventory與獨立裁決確認：M1既有typed customer／staff／admin binding、review、owner projection、aggregate readback、revocation及同型replacement route／application；但目前單列identity root不能持久化合法customer+staff雙角色，缺role context／Rich Menu switch-restore、staff retirement接線與binding failure→既有durable Customer Service escalation整合。同型replacement另有已確認的repository guard缺陷。M3／M4已有recipient／snapshot／token／delivery及generic terminal/config骨架，但candidate contact尚未收斂到canonical delivery lineage，`LINE-006`缺LINE-owned typed current Query／root key／reason／terminal predicate／fresh readback。 | 雙角色持久化需要schema／migration，依後續邊界控制標`BOUNDARY_REQUIRED`，不得自行施工。同型replacement兩檔候選已在獨立worktree完成fail-before-fix與focused `23 passed`，但新regression需要改至新Module canonical test root；因超出該slice已宣告write set，維持`ARCH_MAP_GAP`／`TEST_OWNER_GAP`且未commit／未整合。下一個material gate是人工確認這個最小test-routing write-set變更，以及另行裁決M1 schema與其餘owner semantics；不得外推provider／LIFF E2E通過。 |

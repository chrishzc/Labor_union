...
# 85 訂單狀態與月嫂歷史配對 Web 過渡匯入 Work Package
...
4. Apply 每列 fresh-lock Order，唯一匹配為 `case_no + client_name`；0／1／2 採納歷史 asserted
   status，符合格式的來源歷史 status／日期直接寫入，不以來源更新時間或目前值衝突阻擋。空白／未知值
   進 review。案件編號缺失或 unmatched case 是零 mutation、零 anomaly 的 terminal outcome。
...
7. matched review row 建立 immutable Orders review evidence，並以 `HISTORICAL-ORDER-001` current-state
   warning 投影至異常警示中心；fingerprint 為 review identity，只顯示遮罩案件識別與 issue codes。
...
- `06_Anomalies_Domain.md`、Orders historical review outbox consumer、anomaly registry 與 worker wiring
...
## 8. 2026-08-14 completion

使用者已裁決以版本庫內去敏 workbook 作為受控來源完成證據，且 UI 驗收可使用內建瀏覽器；既有
Chrome／Streamlit receipt 亦已覆蓋 matched、unmatched、review、replay 與 conflict 顯示。WP80
後續已完成 pre-190 → part 190 preserve-data candidate、資料保留與 exact descriptor gate，因此第 6.5、
6.7 的跨包依賴已解除。

本包完成。日常管理入口為資料匯入中心 Orders typed card；operator-only CLI 的最終移除仍須依
entry-point governance 另行裁決，不屬本包 completion blocker。

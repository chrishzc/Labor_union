...
2. HCM adapter 停止 fabricated defaults；案件編號具備正式建案資格時，仍建立完整 Client／Order 列，
   可解析欄位寫入、錯誤欄位維持 `NULL` 並建立欄位級 warning，Order 固定 `待補件` 且不得建立帳務
   root facts。案件編號缺失或不符合建案資格時不得建立 Client／Order，只保留 privacy-safe review。
...
1. HCM 有可用案件編號時建立正式 Client／Order；無效欄位保持 `NULL`、不存在 fabricated date／service
   terms，Order 為 `待補件` 且不得進入帳務。只有案件編號缺失／不合格時不建案並保留 review。
...
`tests/test_case_import_disposable_mysql_e2e.py` 已以合成有效 HCM workbook
穿過 coordinator、intake 與 Case Import workflow，驗證 Preview `ready_count=1`、Apply 建立 `洽談中`
正式案件與 bootstrap event；`tests/test_hcm_import_api_disposable_mysql_e2e.py` 再穿過 authenticated
multipart route 驗證 Preview、Apply、same-key replay 與 changed-payload conflict。同檔也保留待補件列
不建立帳務義務的驗證。HCM review 的 Query／Correct／Reject 與警示中心轉介已依後續人工裁決移至
WP86／警示中心後續任務，不屬本包 completion gate。使用者亦已允許 Chrome 不可用時以內建瀏覽器
完成 UI 驗收。

### HCM 歷史過渡入口實作（2026-08-14）

已新增 `/api/v1/case-import/hcm/historical-workbooks/preview` 與 `/apply`，沿用同一 workbook
Preview／Apply、idempotency claim 及 terminal receipt 合約，但使用獨立 historical intake。來源列依
`報名時間(建檔)` 由舊到新處理；既有案件直接覆寫 HCM 所有可更新欄位及可解析的服務條款
（天數、開始／結束日、每日時數、起訖時間與跨日標記），
不覆寫 `orders.status`，也不寫帳務、薪資、排程或配對根事實。無案件編號或無法解析來源業務時間的列
不建案並保留 HCM review；其他欄位無效時維持 `NULL` 與 warning。實際 MySQL 驗證見
`tests/test_case_import_disposable_mysql_e2e.py` 與
`tests/test_hcm_import_api_disposable_mysql_e2e.py`：合成同案逆序來源最終留下較新姓名與服務天數，
並保留 `洽談中` 訂單狀態。

### Completion（2026-08-14）

- focused HCM parser／coordinator／route／client／entry guard：`54 passed, 3 xfailed`；三個 xfail 只保留
  historical production importer 的 true no-write characterization，不是 Web upload 行為缺口。
- disposable MySQL：`10 passed`，包含完整建案、髒列整列寫入且錯誤欄位 `NULL`、`待補件` 帳務隔離、
  authenticated multipart Apply／replay／changed-payload conflict，以及 HCM 歷史過渡覆蓋。
- 既有內建瀏覽器 receipt 已驗證 upload、invalid review、replay、conflict 與 temporary-file cleanup；
  使用者已接受該 UI 證據。

WP73 第一階段完成。Client／Staff Web cards 仍由 WP83 承接；警示中心人工處置由 WP86／後續包承接；
ADR-001 其他長期重構不因本包完成而自動完成。

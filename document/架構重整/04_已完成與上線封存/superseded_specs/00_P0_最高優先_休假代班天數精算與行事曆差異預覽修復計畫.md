...
- 狀態：`superseded-core-completed-residual-tracked`
- 實作狀態：`implemented-and-ui-accepted-by-WP66-WP67-WP69-WP70`
...
- 更新日期：2026-08-13
- 實作授權：`historically-completed-by-approved-work-packages`
...
- Calendar Preview、獨立 Apply readiness、Holiday Query、服務天數守恆、順延／代班 day-cell、
  fresh Apply、跨 Domain transaction 與 Chrome 行事曆驗收，已由 WP66、WP67、WP69、WP70 完成。
- 現行契約由 `01_規格基線/02_Assignments_Scheduling_Domain.md` 承接，本計畫不再作為開工入口。
- Client Finance／Payroll bootstrap blocker 的一般人工修正入口尚未完成，已轉列
  `02_決策與退役執行記錄/58_未實作_未落地_未上線規格總表.md`；不得因本文件封存視為完成。
- 月嫂透過 LINE 提出請假申請及管理待辦，仍由
  `document/功能開發計畫/Scheduling_月嫂請假申請待辦與管理端處理開發計畫.md` 擁有。

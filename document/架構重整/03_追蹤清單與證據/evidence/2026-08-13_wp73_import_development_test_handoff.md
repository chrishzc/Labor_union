...
補充包只包含程式、規格、part 189 release與去敏測試；不包含`.env`、真實來源Excel或WP79／LINE
...
第一條必須列出 `labor-union-wp77-2026-08-13-v1` 與 part 189；第三條必須成功。若回
...
預期`release_id=labor-union-wp77-2026-08-13-v1`，最後一個artifact為
`189_staff_historical_adoption_hcm_review.sql`；命令仍報hash mismatch時停止，不連 DB。

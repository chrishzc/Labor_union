...
- 生效結果：本文件與 `16`～`18` 已經人工整體確認，成為後續 Work Package 的
  業務與架構依據。
- 本次核准只授權建立 Inventory v2；不授權 production code、pytest、schema、
...
3. 正式狀態、金額、服務日與權限判斷只在後端 owning Domain。
4. 外部 webhook、LINE、BreezySign、File Watcher 與 UI 不得直接寫 Orders、
...
6. 管理操作同時要求 service authentication、administrator authentication 與
   operation capability；audit log 不是授權依據。
...
| Contract Integration | BreezySign contract identity、evidence inbox、processing receipt 與 evidence outbox | `17_External_Integration_LINE_Access正式規格.md` |
...
| Access Control | 管理員、session、capability 與安全稽核 | `17_External_Integration_LINE_Access正式規格.md` |
...
flowchart TB
  ...
  AC -->|"Authenticated Principal + Capability"| COORD
...
- 管理／業務 API 必須先經 Access Control authentication／operation capability gate，
  再呼叫 typed Application Command／Query。
- LINE／BreezySign webhook 通過 request security 驗證後，只建立 canonical durable
...
- Contract Integration 只把 verified evidence 提供 Orders；LINE 只負責平台互動。
...
| CON-REF-001 | 一般客戶退款 | 納入 Client Finance 正式目標能力；允許多筆有效出款逐步清償，progress 與 review 分成正交投影；不允許負收款或改寫原交易；目前實作仍為 `partial` | `consolidated-decision` |
| CON-REF-002 | 退款與沖正 | refund 清償退款義務；reversal 使既有正式收款失效並重開原應收，兩者不可共用命令 | `confirmed-inherited` |
| CON-BS-001 | BreezySign webhook | Integration 只保存並驗證外部簽署證據；不得猜日期、改 Orders status 或建立 assignment | `consolidated-decision` |
...
| CON-AUTH-001 | 管理權限 | `AdminPrincipal + capability` 是正式授權；internal key 只是 service authentication | `consolidated-decision` |
| CON-DEP-001 | 部署 | 固定安全拓撲與部署契約；`local-primary` 僅為 `recommended-candidate`，實際 profile 與 edge provider 必須由 deployment decision record 人工選擇 | `consolidated-decision` |
...
- `05_潛在狀態機規則盤點.md`：`source-analysis`；已收斂項由本總表與 `16`～`18` 覆蓋。
...
- `08_非Markdown附件索引.md`：`attachment-evidence-index`；
  `human-content-review-required` 未完成前，不宣稱整個 `document/` 來源覆蓋完成。
...
下列項目不阻擋本次邏輯架構完成，但阻擋相應 release：

1. 是否採用 `local-primary` recommended candidate 及其 24/7 readiness；若不採用，須另行決定切換
   `hybrid-app-host`；
2. public edge provider 與正式網域；
3. RTO、RPO、備份保存期、maintenance window 與 on-call owner；
4. LINE 逾時 review 的轉派時限與 Rich Menu 發布雙人覆核政策；
5. operation capability 已是最終授權依據；仍需決定 role bundle 是固定於程式，
   或允許 system-admin 以 versioned policy 管理 capability grants。
...
- 一次性核准狀態同步已完成並 freeze；approval receipt 固定於
  `document/架構重整/evidence/architecture_approval_2026-08-03.json`；
- Inventory v2 active exact write set 只限：
  `document/架構重整/evidence/writer_inventory_v2/README.md`、

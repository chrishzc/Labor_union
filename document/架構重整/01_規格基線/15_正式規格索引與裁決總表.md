...
- 2026-08-13 WP77 最新裁決：Staff restricted historical source在 identity、姓名一致且來源報名時間
  嚴格較新時覆寫可更新 scalar；來源空值與受保護 root fields 不覆寫，銀行／關聯仍保守合併；
  HCM以案件編號防重；新案件若IP位址與姓名同時命中既有Client，視為疑似同一客戶重複申請，
  不載入並通知公會人工確認；只有IP相同但姓名不同不阻擋。HCM與Client BeClass可獨立匯入，
  缺少對方只形成current-state anomaly，待兩方唯一配對後才綁定並補入料理條款：`approved`

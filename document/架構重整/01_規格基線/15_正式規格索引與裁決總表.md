...
- 六頁已接入 typed backend result，但整體 Chrome 驗收仍為 `in-progress`，不得宣稱全部完成。
- Data Browser 六來源 Query、遮罩、錯誤復原已完成；其餘營運頁仍須逐頁驗證正常、empty、error、retry 與 mutation。
- HCM Current、Client BeClass、Staff Historical、Historical Orders 已有 Preview／Apply 程式路徑；四類 Apply
  的同檔案 fingerprint、terminal receipt／replay 與 Chrome 操作證據仍須逐類確認後才可標 completed。
...
- React current-calendar 已對目前已載入的每位服務人員各查一次 bounded server projection，選取列不再重複
  ...
  scheduling focused `12 passed`、build／lint PASS。
...
- 靜態盤點仍保留 Orders card projection 內部 `unavailable／blocked` 被誤顯示為空資料、Matching
  `activePlan` 非 404 failure 被吞掉及 OrderTracker settlement fallback 等 runtime corrective todo；不得歸類
  為單純視覺漂移。
- 最近一次 React full suite `131 files / 811 tests`、TypeScript no-emit 與 production build 是前一候選狀態；
  後續 corrective edit 會使其失效，必須在最後相關修改後重跑。

Current todo 依序為其餘 Operations runtime corrective、以既有完整 canonical 測試案件完成 mutation／跨頁流程、
六頁 Chrome 驗收，最後才是 UI 視覺對齊與效能改善。不得對已有 effective Scheduling lineage 的 legacy 案件

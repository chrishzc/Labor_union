...
- 2026-08-14 WP77 最新裁決：Staff restricted historical source以身分證唯一定位；姓名缺失維持既有
  不可建檔規則。來源報名時間嚴格較新時覆寫可更新 scalar；姓名變更可寫入但保留追溯警示，身分證、
  LINE ID、status等受保護 root fields 不覆寫。較新歷史列的銀行／勾選關聯視為完整快照，整組取代舊集合。
  只要任一欄位缺漏或格式無效，即使 Staff 可建立／更新，也必須建立該欄位警示。
  HCM以案件編號為最低寫入資格與防重鍵；有案件編號的列一律建立或補齊正式案件，即使其他欄位
  缺漏、格式錯誤或關聯歧義，並以欄位／關聯警示追補。只有案件編號缺失或不可用才不建案、僅留
  警示。HCM歷史過渡列依來源業務時間由舊至新直接寫入可更新欄位，不另外推導DB目前有效值。
  HCM與Client BeClass可獨立匯入，缺少對方只形成current-state anomaly，待兩方唯一配對後才綁定
  並補入料理條款：`approved`
...
- 2026-08-14 訂單狀態與月嫂歷史配對過渡裁決：舊 `historical_orders` 僅保留 source lineage；
  ...
  服務日期。符合既有精確匹配與資料格式規則的來源歷史值直接寫入，不以來源更新時間或目前 Order
  值阻擋；案件編號缺失或無法匹配時零寫入、零警示。多月嫂缺個別區間不得猜 assignment／日期／薪資；
  日常 UI 不得呼叫 legacy CLI：`approved`
- 2026-08-14 `HISTORICAL-ORDER-001`：歷史訂單 matched review 的 canonical current-state warning
  歸 Orders；以 immutable review identity fingerprint，只顯示遮罩案件與 issue codes，unmatched case
  不建立 anomaly：`approved`
- 2026-08-14 匯入異常外部確認裁決：HCM、Client／Staff BeClass、歷史訂單與銀行來源的原始匯入
  事實不可由警示中心直接改值或重載。警示中心只追蹤公會聯絡來源當事人的處理狀態、去敏摘要與
  terminal disposition；正確資料必須重新走該來源的 typed intake，或由既有 owning Domain typed command
  依外部銀行等根事實建立新 immutable result。未知 recipient 不得自動傳 LINE：`approved`
- 2026-08-14 WP88 匯入警示追蹤裁決：每個 `logical code + field path` 是可獨立解除的 warning item，
  UI 可依案件／來源分組但不得整案一起消除。追蹤狀態為 `open`、`awaiting_external_confirmation`、
  `response_recorded`、`reimport_requested`、`closed`、`auto_resolved`；人工 `closed` 只代表外部追蹤結束，
  `auto_resolved` 只能由 owning Domain root predicate 產生。顯式關聯的新來源仍不合格時建立新 occurrence，
  新警示取代 current task 並由 system 關閉舊 task；exact replay 不建立新警示，所有來源、issue codes、
  occurrence 與狀態事件永久保留。警示中心不得接受 corrected payload 或直接修改 root：`approved`
- 2026-08-14 Client BeClass 過渡綁定裁決：`query_no` 是來源流水號，不得作為客戶識別或案件編號。
  LIFF 啟用前，只在姓名與手機號碼均完全一致且唯一命中正式 Client 時定位客戶；案件候選也必須唯一
  才可綁定，否則留警示、不猜測。LIFF 啟用後由登入身分直接綁定，不再使用此過渡比對：`approved`
...
| Internal Access | 內部使用者登入、session、actor identity 與安全稽核 | `17_External_Integration_LINE_Access正式規格.md` |
...
- 管理／業務 API 必須先驗證 internal-user session 與 enabled 狀態，再呼叫 typed Application
  Command／Query；不得依人員 role／capability 形成業務功能差異。
...
| CON-AUTH-001 | 內部存取 | 所有 authenticated、enabled `AdminPrincipal` 具有相同業務功能權限；principal 保留作 actor／audit，internal key 只作 service authentication | `reconfirmed-by-user-2026-08-12` |

...
- `requires_cooking` 與其他 Orders Terms 一樣只是 Matching 使用的案件條件；來源尚未確認時為
  `NULL`，唯一來源對應後才可由 typed Orders command 更新。來源空白、矛盾或自由文字進 review，
  不得阻擋或回滾其他已成立的 HCM／BeClass 根事實；服務資料鎖形成後不得修改。

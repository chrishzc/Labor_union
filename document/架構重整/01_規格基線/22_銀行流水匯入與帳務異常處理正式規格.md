...
### 13.4 React successor 與驗收狀態（2026-08-23）

- Backend／既有 Streamlit 已具正常銀行檔案 Ingest、Batch Preview、durable Apply、Finance-only job Query
  與 same-key durable replay；React `#finance` 目前仍只有 batches／manifest／review rows／reprocess runs Query，
  Upload／Preview／Apply／Reprocess 均維持 disabled，狀態為 `react-successor-partial`。
...
- 現行 Finance job success public outcome 只含 closed `result_reference`，尚未公開
  `FinanceImportBatchReceiptView` 的 reconciled／existing／pending／resulting-version 欄位，也沒有
  server-authoritative replay marker；因此不得把 JobAccepted、terminal status 或 manifest readback 冒充正式 Apply receipt。
- 既有合法 fixture 只形成 `manual_review`，尚不能證明 `apply_allowed=true` 的 positive Apply。完成 React
  Upload → Preview → Apply → terminal receipt → canonical readback → same-key replay 前，必須先補齊
  Finance terminal receipt contract，並以核准的 ready-dispatch 去識別 fixture，或在受控 `lu_test_*`
  scenario 建立相符 obligation 後執行驗收。
- 本次狀態校正未變更 schema、DB、provider、deployment 或 entry switch；React mutation 與 positive Apply
  仍不得宣稱完成。

...
- React `#finance` 已接正常銀行檔案 Ingest、Batch Preview、confirmed durable Apply、same-key replay 與
  batch terminal receipt re-query。正常頁依第 5.1、11 節固定收斂為「上傳檔案 → 預覽 → 匯入完成」；
  不顯示歷史 batches／manifest／review rows／reprocess runs，待處理筆數大於零時導向異常中心。
...
- 2026-08-25 已在真 FastAPI 與受控 `lu_test_dataset_contract_signing_v4` 由 Chrome 實點完成
  ...
  `finance-import-row:101` 的 current root fact readback 為 `predicate_active = false`、
  `workflow_status = resolved`，未使用通用「確認排除異常」硬改狀態。本 slice 狀態為
  `completed-runtime`。

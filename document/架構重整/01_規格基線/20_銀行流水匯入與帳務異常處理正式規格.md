...
| Anomalies | 根事實異常投影、目前 predicate、事件歷程、認領狀態與可用修復 action |
...
- 原始匯入檔案 digest、sheet／row occurrence 與 canonical finance import row；
...
| 重複匯入／格式錯誤／證據不足 | 修正來源或確認 occurrence，不建立帳務結果 | bank fact／review 保留，正式 ledger 零新增 |
...
```text
open → claimed → root_fact_corrected → auto_resolved
  └───────────────────────────────→ open（修復失敗或 blocker 仍存在）
```

- `claimed` 只表示責任歸屬；
- 手動 `resolved` 不得使 active predicate 消失或修改正式帳務；
- Apply 成功後必須重新投影 root facts；predicate 消失才可 `auto_resolved`；
- 同一異常再次發生時使用新 occurrence 並 reopen，不覆寫歷史。

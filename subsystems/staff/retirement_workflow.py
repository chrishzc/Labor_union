...
Description: 協調 Staff lifecycle 的 Query、Preview、Apply 與冪等交易。
"""
...
class StaffLifecycleWorkflow:
    ...
    def apply(self, request: StaffLifecycleApplyRequest) -> StaffLifecycleReceipt:
        command_fingerprint = fingerprint_payload({"staff_id": request.staff_id, "transition": request.transition.value, "effective_at": request.effective_at.isoformat(), "reason_code": request.reason_code, "expected_version": request.expected_version.value, "actor": request.actor.actor_id})
        with self._unit_of_work_factory() as unit_of_work:
            ...
            receipt = StaffLifecycleReceipt(fact.staff_id, preview.candidate.after.state, preview.candidate.after.version, preview.fingerprint)

"""Bounded, typed read contract for the Staff roster case-preference projection."""
...
_CANONICAL_VALUE_ORDER: dict[str, tuple[str, ...]] = {
    ...
    "service_periods": ("4小時(上午8:30-12:30)", "4小時(下午13:00-17:00)", "8小時", "24小時"),
...
_OTHER_VALUE_MARKERS = frozenset({"其他", "其它"})
_SOURCE_NOT_READY_TOPICS = frozenset({"transportation"})
...
class StaffCasePreferenceSummaryQueryService:
    """Project Staff-owned relation facts into the roster-only preference read model."""
    ...
    def query(
        ...
        if not isinstance(topic_rows, Mapping) or set(topic_rows) != set(_TOPIC_ORDER):
            raise StaffCasePreferenceSummaryContractError("repository topic set is not canonical")
...
def _topic_summary(
    ...
    for row in rows:
        if not isinstance(row, Mapping) or set(row) != {"value", "other_detail"}:
            raise StaffCasePreferenceSummaryContractError(f"{topic} repository row fields are not canonical")
        value = _required_text(row["value"], f"{topic} value")
        detail = _optional_text(row["other_detail"], f"{topic} other_detail")

        if value in _OTHER_VALUE_MARKERS:
            if topic not in _SOURCE_NOT_READY_TOPICS and detail is not None:
                detail_values.add(detail)
            continue

        if detail is not None:
            raise StaffCasePreferenceSummaryContractError(
                f"{topic} non-other value must not carry other_detail"
            )
        values.add(value)
    ...
    ordered_values = _ordered_values(topic, values)
    if topic in _SOURCE_NOT_READY_TOPICS:
    ...
    other_detail = next(iter(detail_values), None)
    return PreferenceTopicSummary(
        values=ordered_values,
...
def _required_text(value: object, label: str) -> str:
    ...
    if len(normalized) > 255:
        raise StaffCasePreferenceSummaryContractError(f"{label} exceeds maximum length")
    return normalized
...
class StaffCasePreferenceSummaryQueryApplication:
    ...
    def __init__(self, service: StaffCasePreferenceSummaryQueryService) -> None:

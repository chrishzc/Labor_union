...
def _validate_optional_workers(environment: Mapping[str, str]) -> dict[str, bool]:
    contract_enabled = _is_true(
        environment.get("CONTRACT_INTEGRATION_RUNTIME_ENABLED", "false")
    )
    knowledge_enabled = _is_true(
        environment.get("KNOWLEDGE_RETRIEVAL_RUNTIME_ENABLED", "false")
    )
    if contract_enabled:
        _require_secret(environment, "BREEZYSIGN_WEBHOOK_SECRET")
        _require_secret(environment, "BREEZYSIGN_SIGNATURE_HEADER")
        _require_json_object(environment, "BREEZYSIGN_STATUS_MAP_JSON")
    if knowledge_enabled:
        _require_nonempty(environment, "KNOWLEDGE_CHROMA_PATH")
    return {"contract_integration": contract_enabled, "knowledge_retrieval": knowledge_enabled}

...
class QueueLineRichMenuPublicationCommand:
    ...
    correlation_id: CorrelationId
    preview_id: int
    preview_config_revision: str
    preview_config_fingerprint: str
    previewed_by_admin_user_id: int

    def __post_init__(self) -> None:
        _validate_menu_definition_id(self.menu_definition_id)
        require_positive_integer(self.preview_id, "LINE Rich Menu preview ID")
        require_canonical_text(
            self.preview_config_revision, "LINE Rich Menu preview revision", 64
        )
        require_canonical_text(
            self.preview_config_fingerprint,
            "LINE Rich Menu preview fingerprint",
            64,
        )
        require_positive_integer(
            self.previewed_by_admin_user_id,
            "LINE Rich Menu preview administrator ID",
        )

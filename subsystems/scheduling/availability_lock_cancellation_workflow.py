...
def _envelope_value(command_envelope: Any, field_name: str) -> Any:
    if isinstance(command_envelope, dict):
        if field_name not in command_envelope:
            raise ValueError(f"command_envelope is missing {field_name}")
        return command_envelope[field_name]
    if not hasattr(command_envelope, field_name):
        raise ValueError(f"command_envelope is missing {field_name}")
    return getattr(command_envelope, field_name)


def _load_preflight_active_lock(
    cursor: Any, case_no: str
) -> dict[str, Any] | None:
    cursor.execute(
    ...
    pre_lock_value = cursor.fetchone()
    if pre_lock_value is None:
        return None
...
def cancel_caregiver_availability_lock_for_order(
    ...
    **kwargs: Any,
    ...
    if not callable(getattr(cursor, "execute", None)):
        raise ValueError("cursor must provide execute")
    if not callable(getattr(cursor, "fetchone", None)) or not callable(
        getattr(cursor, "fetchall", None)
    ):
        raise ValueError("cursor must provide fetchone and fetchall")
    request = _normalize_request(case_no, event_key, actor, cancel_reason, **kwargs)
    if _envelope_value(command_envelope, "cursor") is not cursor:
        raise ValueError("cursor must be the command envelope cursor")
    envelope_case_no = _strict_str(
        _envelope_value(command_envelope, "case_no"),
        "command_envelope.case_no",
    )
    if envelope_case_no != request["case_no"]:
        raise ValueError("case_no must match the command envelope")

    existing_event = _load_event_for_key(cursor, request["event_key"])
    if existing_event is not None:
        payload = _load_event_payload(existing_event["payload"])
        payload_plan_id = _positive_int(
            payload["plan_id"], "event_payload.plan_id"
        )
        payload_lock_id = _positive_int(
            payload["lock_id"], "event_payload.lock_id"
        )
        existing_lock_staff_ids = [
            _positive_int(value, "event_payload.staff_ids")
            for value in _row_list(
                payload["staff_ids"], "event_payload.staff_ids"
            )
        ]
        ...
        canonical_staff_ids = sorted(existing_lock_staff_ids)
        locked_staff_ids = lock_staff_occupancy_mutex(
            cursor, canonical_staff_ids
        )
        if locked_staff_ids != canonical_staff_ids:
        ...
        locked_event = _load_event_for_key(
            cursor, request["event_key"], for_update=True
        )
    ...
    canonical_staff_ids = preflight["plan_snapshot"]["staff_ids"]
    locked_staff_ids = lock_staff_occupancy_mutex(
        cursor, canonical_staff_ids
    )
    if locked_staff_ids != canonical_staff_ids:
    ...
    _assert_exact_state_match(
        preflight["plan_snapshot"], state["plan_snapshot"]
    )
    ...
    if (
        state["lock_row"]["status"] != "active"
        or state["lock_row"]["is_active"] != 1
        or state["lock_row"]["released_by"] is not None
        or state["lock_row"]["released_at"] is not None
    ):
        raise ValueError("lock is not a complete active component")

    active_days = [
        row for row in state["lock_days"] if row["active_marker"] == 1
    ]
    if len(active_days) != len(state["lock_days"]):
        raise ValueError("lock component contains partially released days")
    if len(active_days) != len(preflight["active_lock_rows"]):
        raise ValueError("active lock rows mismatch before cancellation")
    if any(
        row["released_by"] is not None or row["released_at"] is not None
        for row in active_days
    ):
        raise ValueError("active lock day contains release metadata")
    ...
    cursor.execute(
        ...
        "SET active_marker = NULL, released_by = %s, "
    ...
    if cursor.rowcount != 1:
        raise ValueError("lock event insert rowcount mismatch")
    return {
        "result": "cancelled",
        "case_no": request["case_no"],
        "plan_id": state["plan_row"]["id"],

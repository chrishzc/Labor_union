"""Rollback-only Finance Import diagnostic for historical workbook checks.

Canonical Finance Import ingestion lives in :mod:`subsystems.finance_import.ingestion`.
The diagnostic preserves no root facts and always rolls its transaction back.
"""
...
def allocate_receipt(
    receivables: dict[str, Any], current_state: dict[str, Any], amount: Any
    ...
    try:
        with connection.cursor() as cursor:
            staging = stage_finance_rows(
                cursor, normalized_result, load_finance_identity_maps(cursor)
            )
            result = _complete_diagnostic_dry_run(cursor, staging)
        manifest = _diagnostic_manifest(source_path, normalized_result, staging, result)
        connection.rollback()
        return manifest
    except Exception:
        connection.rollback()
...
def _complete_diagnostic_dry_run(cursor: Any, staging: dict[str, Any]) -> dict[str, Any]:
    ...
    for staged_row in staging["staged_rows"]:
        manifest = _row_manifest(staged_row)
        if staged_row.get("result") == "skipped_existing":
            skipped_existing += 1
            row_results.append(manifest)
            continue
        inserted_rows += 1
        row_id = int(staged_row["row_id"])
        dispatch = dispatch_finance_import_row(cursor, row_id, batch_id)
        manifest.update(
            dispatch_result=dispatch["result"],
            reason=dispatch.get("reason"),
            finance_alert_action=dispatch.get("finance_alert_action"),
        )
        row_results.append(manifest)
        if dispatch["result"] in {"reconciled", "existing"}:
            classification = str(staged_row.get("classification_type"))
            reconciled_counts[classification] = reconciled_counts.get(classification, 0) + 1
        else:
...
def _row_manifest(staged_row: dict[str, Any]) -> dict[str, Any]:
    return {
...
def _diagnostic_manifest(
    ...
    return {
        ...
        "reconciled_counts": result["reconciled_counts"],
        "pending_rows": [],

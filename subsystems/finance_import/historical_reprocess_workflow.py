...
class HistoricalReprocessRepository(Protocol):
    def load_historical_reprocess(self, batch_identity: str, *, for_update: bool) -> HistoricalReprocessFacts: ...
...
class HistoricalReprocessWorkflow:
    ...
    def preview(self, batch_identity: str, correlation_id: CorrelationId) -> HistoricalReprocessPlan:
        try:
            return build_historical_reprocess_plan(self._repository.load_historical_reprocess(batch_identity, for_update=False))
    ...
    def _fresh_plan(self, request):
        facts = self._repository.load_historical_reprocess(request.batch_identity, for_update=True)
...
def _command_fingerprint(request: HistoricalReprocessApplyRequest) -> PreviewFingerprint:
    return fingerprint_payload({"batch": request.batch_identity, "version": request.expected_batch_version.value, "preview": request.preview_fingerprint.value, "actor": request.actor.actor_id, "reason": request.reason})

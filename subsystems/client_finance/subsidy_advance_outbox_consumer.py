"""Consume government allocation outbox facts for subsidy-advance recovery."""
...
def consume_government_subsidy_advance_events(connection, maximum_events: int = 50) -> tuple[int, int]:
...
def _consume_event(connection, event) -> None:
    ...
    workflow = SubsidyAdvanceRecoveryWorkflow(
        MySqlSubsidyAdvanceRecoveryRepository(connection)
    )

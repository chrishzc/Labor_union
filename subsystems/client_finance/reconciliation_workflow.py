...
class ReconciliationSelection:
    case_no: str; payment_stage: PaymentStage; bank_fact_identities: tuple[str, ...]; obligation_identities: tuple[str, ...]
...
class ClientReconciliationWorkflow:
    ...
    def apply(self, request: ClientReconciliationApplyRequest) -> ClientReconciliationReceipt:
        ...
        with self._unit_of_work_factory() as unit:
            ...
            (self._repository.append_orders_deposit_intent if preview.candidate.status is ReconciliationStatus.EXACT and preview.candidate.payment_stage is PaymentStage.DEPOSIT else self._repository.append_anomaly_intent)(preview.candidate)
...
def _preview(selection, facts):
    _facts(selection, facts); candidate=build_reconciliation_candidate(facts.bank_facts, facts.obligations)
...
def _selection(s): return {"case_no":s.case_no,"payment_stage":s.payment_stage.value,"bank_fact_identities":s.bank_fact_identities,"obligation_identities":s.obligation_identities}

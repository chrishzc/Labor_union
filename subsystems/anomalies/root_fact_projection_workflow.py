...
from domains.anomalies.registry import AnomalyDefinitionRegistry, CurrentAlertProjection, reduce_current_alert
...
def _recovery_actions(definition_code: str, projection: CurrentAlertProjection) -> tuple[RecoveryActionLink, ...]:
    if definition_code == "CLIENTREFUND-001":
        return (
            RecoveryActionLink(
                ...
                subject_identity=projection.source_identity,
                subject_version=projection.source_version,
                required_inputs=("evidence", "reason", "refund_ledger_entry_identity", "target_obligation_identities"),
            ),
        )
    if definition_code != "finance_import_manual_review":
        return ()
    return finance_manual_review_recovery_actions(projection.source_identity, projection.source_version)

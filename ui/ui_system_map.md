...
- Dependencies: [ClientPaymentRouter]
...
- Invariants:
  ...
  - 選擇客戶 `reversal` 時必須顯示原收款 selector，只列出同案件、同階段、`transaction_type=receipt`、`transaction_status=succeeded` 且尚有可沖正餘額的交易；提交 payload 必須帶 positive `reversal_of_transaction_id`，不得用金額、日期或 external_reference 猜測。
  - 選擇 `receipt` 時不得傳 reversal_of_transaction_id；UI 只建立 succeeded canonical ledger transaction，不提供 failed／reversed attempt 狀態。
  - deposit POST 必須使用本頁 orders_data 對應案件的 non-negative lifecycle_version 作 `lifecycle_expected_version`；缺值時阻止提交並要求重新整理，不得以 0 或目前畫面狀態猜測。first/second stage 不傳該欄位；409/400 conflict 顯示重新整理提示。
- Verification:
...
##### Module: OrderLifecycleAdminApiClient
- Sub Map: ui_layer
- Type: api_client
- State: `planned`
- Source: ui/api_clients/order_lifecycle_api_client.py
- Dependencies: [OrderRouter, OrderSchemas, UIAdminApiContext]
- Description: 為 Calendar 封裝 lifecycle-control-state 查詢與 actual-start reconfirm command；集中處理管理員認證、timeout、BaseResponse、typed 409 與 transport error，不包含 Streamlit 或商業推導。
- Complexity: medium
- Input:
  - control_state_query: canonical case_no。
  - actual_start_reconfirmation_command: expected_version、idempotency_key、actor、reason、new_actual_start_date，以及同一 Calendar Preview 的完整 order_change、assignment_plan、schedule_change_plan。
- Output:
  - lifecycle_control_state: authoritative lifecycle_version、canonical status、actual-start control active/cleared、required/current date、blockers、can_reconfirm。
  - lifecycle_command_result: persisted_version、decision 與 control/history/outbox identities。
  - typed_error: 保留 HTTP status、stable machine code、message 與 retryable。
- Algorithm:
  - GET `/api/v1/orders/{case_no}/lifecycle-control-state` 取得 authoritative snapshot；POST `/api/v1/orders/{case_no}/actual-start/reconfirm` 提交完整 command。
  - 使用 UIAdminApiContext 建立正式 headers 與 canonical actor；集中套用 timeout、解析 BaseResponse 與 `{code,message}` conflict detail。
  - transport、invalid response 與未知 server error 轉為穩定 typed error；不得 fallback 到 services、DB 或 UI 推導。
- Invariants:
  - 不得 import streamlit、services 或 db_service，不得執行 SQL；不得接受或合成 derived blockers、control state、status after value或outbox action。
  - POST payload 的 actor 必須是 UIAdminApiContext 的 canonical username；expected_version 必須直接取自同案件最近一次 control-state response，不得預設為0或由 orders.status 推算。
  - 409 必須保留 `order_lifecycle_version_conflict`、`order_lifecycle_idempotency_conflict`、`order_lifecycle_state_conflict`或`order_lifecycle_blocked`，不得轉成一般成功或空資料。
  - idempotency_key 由呼叫端對同一未變更草稿穩定提供；client 不得每次 rerender 任意換 key。
- Verification:
  - command: {"argv": [".venv\\Scripts\\python.exe", "-m", "pytest", "tests\\test_order_lifecycle_admin_api_client.py", "-q", "-p", "no:cacheprovider", "--basetemp", "C:\\tmp\\pytest-order-lifecycle-admin-api-client"], "cwd": "project", "timeout": 90, "expect_exit": 0, "expect_stdout_contains": "passed"}
- Observability: not_required
...
- Type: ui_page
- State: `planned`
- Source: ui/pages/03_calendar.py::_render_staff_calendar,_render_actual_start_reconfirmation,_render_assignment_leave_resolution,safe_float,safe_int,safe_date,_normalise_calendar_schedule_map,_multi_caregiver_request,_current_admin_actor,_calendar_has_unsaved_leave_changes,_discard_calendar_leave_drafts,_multi_caregiver_error,_coerce_iso_date_strict,_coerce_staff_id,_extract_case_assignments_for_staff
- Description: 服務人員月曆分頁。由 StaffMonthlyCalendarScheduleRouter 載入單月 assignment／等待訂金鎖占用；出勤精算先讀 authoritative lifecycle control，若真正開始日需重確認則以完整同步 Preview 加單一 reconfirm command 原子套用，之後才提供多日期休假逐筆順延／代班 Preview 與 atomic Apply。原「多月嫂指派排班」案件→指派功能模塊已從 live code 移除。
- Dependencies: [StaffMonthlyCalendarScheduleRouter, HolidayRouter, StaffRouter, OrderRouter, OrderScheduleCalculationRouter, UIAdminApiContext, OrderLifecycleAdminApiClient, MultiCaregiverCaseAssignmentListRouter, MultiCaregiverScheduleReadRouter, MultiCaregiverScheduleRouter, AssignmentScheduleRestDateRouter]
- Complexity: medium
- Input:
  - calendar_selection: 已選 staff、case_no、年月與正式 assignment snapshot。
  - actual_start_draft: new_actual_start_date、完整 assignment synchronization target、管理員 reason 與確認意圖。
- Output:
  - actual_start_preview: 同步 Preview 的 before/after、required_schedule_removals、blocking_reasons 與 authoritative lifecycle control state。
  - actual_start_result: 單一 reconfirm command 結果；成功後刷新 control state、order/assignment/calendar，衝突則保留草稿。
- Algorithm:
  - 選定出勤精算案件後，先以 OrderLifecycleAdminApiClient 讀 lifecycle-control-state；只有 actual-start control active 且 can_reconfirm 才顯示可編輯重確認流程，cleared/不適用則不建立 mutation 草稿。
  - 日期或 assignment target 變更時呼叫既有 assignment-synchronization Preview，完整顯示 before/after、移除清單與 blockers；任何 blocker 或非 in_sync 都禁止確認。
  - 對未變更草稿建立穩定 idempotency_key，使用 control response 的 lifecycle_version、canonical actor、非空 reason 與完整 Preview payload，只呼叫 actual-start/reconfirm；不得先呼叫普通 assignment Apply。
  - 成功後清除草稿並刷新 order、assignments、control state 與月曆；409 保留草稿、刷新 authoritative state並顯示 machine code，讓管理員重新 Preview。
- Invariants:
  ...
  - UI 不得由 orders.status、actual_start_date、alert 或畫面暫存推測重確認義務；是否顯示、required/current date、blockers、can_reconfirm 與 expected_version 一律取 authoritative control-state response。
  - 重確認 reason 必填；同一未變更草稿跨 Streamlit rerun 必須重用同一 idempotency_key，任一日期、assignment、schedule removal、actor 或 reason 改變時舊 Preview 與 key 都失效。
  - actual-start/reconfirm 已在同一 Service transaction 擁有 assignment synchronization Apply；Calendar 不得先送普通 Apply、不得雙寫 orders/assignments/schedule，也不得在 command 成功前把草稿顯示為正式資料。
  - 409 時不得丟棄管理員草稿或靜默重送；必須刷新 control state、標示 stable conflict code，要求重新 Preview。transport/500 亦不得假定成功。
- Verification:
  - command: {"argv": [".venv\\Scripts\\python.exe", "-m", "py_compile", "ui\\pages\\03_calendar.py"], "cwd": "project", "timeout": 60, "expect_exit": 0}
  - command: {"argv": [".venv\\Scripts\\python.exe", "-m", "pytest", "tests\\test_calendar_ui_explicit_errors.py", "tests\\test_calendar_actual_start_reconfirmation_ui.py", "tests\\test_multi_caregiver_scheduling_ui_shell.py", "-q", "-p", "no:cacheprovider", "--basetemp", "C:\\tmp\\pytest-calendar-lifecycle-batch"], "cwd": "project", "timeout": 180, "expect_exit": 0, "expect_stdout_contains": "passed"}
- Observability: not_required
...
##### Module: FinanceAlertCenterApiClient
- Sub Map: ui_layer
- Type: api_client
- State: `planned`
- Source: ui/api_clients/finance_alert_center_client.py
- Dependencies: [FinanceAlertCenterContracts]
- Description: 為 Streamlit 警示中心集中封裝 FastAPI base URL、認證、timeout、Query／Command serialization、response envelope 與 typed error parsing；不含 Streamlit 或商業判斷。
- Complexity: medium
- Input:
  - contract_request: FinanceAlertCenterContracts 的 list/detail/claim/resolve/scan Query 或 Command
- Output:
  - contract_view_model: 已驗證的 list/detail/import-review/action ViewModel
  - typed_error: 原樣保留 code、message、field errors 與 retryable
- Invariants:
  - 不得 import streamlit、services、db_service 或讀取本機正式檔案；不得在 HTTP 失敗時 fallback 直連 DB／Service。
  - URL、headers、認證、timeout、response envelope 與 JSON parsing 只能集中在本節點；FinanceAlertCenterUI 不得散落 requests.*。
  - 只驗證與解析 FinanceAlertCenterContracts；不得從 raw details 推導 classification、reconciliation、金額、候選或 alert lifecycle。
  - HTTP validation、authorization、not found、conflict、unavailable 與 internal error 必須保留 typed semantics，不得轉成空清單或成功。
- Algorithm:
  - 將 framework-neutral request 序列化為固定 endpoint/query/body，套用共用認證與 timeout。
  - 驗證 HTTP status 與 response envelope，成功解析 ViewModel，失敗解析 Typed Error。
  - transport/invalid-response 轉為 stable unavailable/internal typed error，不執行 fallback。
- Verification:
  - command: {"argv": [".venv\\Scripts\\python.exe", "-m", "py_compile", "ui\\api_clients\\finance_alert_center_client.py"], "cwd": "project", "timeout": 30, "expect_exit": 0}
- Observability: not_required
...
- Type: ui_page
- State: `planned`
- Source: ui/pages/06_finance_alerts.py::_client,_operator_default,_show_error,_money,_render_summary_table,_render_display_fields,_render_import_review,_render_detail,_render_actions,_render_alert_family,_render_import_tab,show
- Dependencies: [FinanceAlertCenterContracts, FinanceAlertCenterApiClient]
- Description: 提供跨 CLIENT、RETURN、SUBSIDY、STAFF、COMMON 的財務警示與資料匯入異常中心，供人工檢視、認領與解除，不直接操作正式帳務。
- Complexity: medium
- Input:
...
- Invariants:
  - UI 只可透過 FinanceAlertCenterApiClient 提交 FinanceAlertCenterContracts Query／Command；不得直接使用 requests、Router、services、db_service 或 SQL。
  ...
  - 「資料匯入異常」必須接上 IMPORT-006「銀行對帳匯入待人工分類」，並以 batch-level details 顯示 occurrence/distinct/remaining、direction/reason counts、最多 20 個 sample row ids 與最近 reprocess 摘要。
  - UI 文案必須區分：IMPORT-006 表示尚未分類成業務事件；已成為業務分類但下游核銷 pending 則顯示於 finance alerts。不得把兩者合併成已入帳或已核銷。
  - 不提供姓名自動匹配、刪除 staging、force reconcile 或直接執行 reprocess apply 的按鈕；歷史重處理保持人工 CLI。
  - 頁面載入只讀已物化 system_alerts，不得每次 render 掃描數千筆 finance_import_rows；重新掃描只能由使用者明確按鈕呼叫 POST scan。
  - UI 只渲染 ViewModel 與 Typed Error；不得解析 system_alerts.details、finance_alerts candidate JSON 或依 DB 欄位推導業務狀態。
- Algorithm:
  - 以獨立 Streamlit 頁面透過 FinanceAlertCenterApiClient 載入財務稽核警示及匯入／流程目前狀態 ViewModel。
  - 資料匯入分頁列入 IMPORT-006；選取後以 bounded batch summary 顯示原因分布，不載入 raw payload 或完整帳號。
  - 對選定警示呼叫 claim 或 resolve API，顯示 conflict 與 invalid transition；明確重新掃描時才呼叫 system alert scan。
- Verification:
  - command: {"argv": [".venv\\Scripts\\python.exe", "-m", "py_compile", "ui\\pages\\06_finance_alerts.py"], "cwd": "project", "timeout": 60, "expect_exit": 0}
- Observability: not_required

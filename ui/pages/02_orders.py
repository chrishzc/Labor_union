...
def show():
    ...
    try:
        orders_data, staff_list = _load_initial_lists()
...
def _load_initial_lists():
    ...
    with ThreadPoolExecutor(max_workers=INITIAL_QUERY_WORKERS) as executor:
        ...
        staff = executor.submit(
            _request_list,
            f"{base_url}/api/v1/staff",
            header_items,
            "月嫂",
        )
    return _resolve_order_summary(orders, cursor, cached), staff.result()


def _request_order_summary(base_url, header_items, cursor, etag):
...
def _cached_order_summary(cursor):
    cache = st.session_state.get("orders_summary_cache", {})
    if not isinstance(cache, dict):
        return None
    candidate = cache.get(cursor or "")
...
def _resolve_order_summary(future, cursor, cached):
    if future is None:
    ...
    if result.not_modified:
        ...
        return list(cached["items"])
    return _store_order_summary(cursor, result)


def _store_order_summary(cursor, result):
...
def _render_order_summary_pagination() -> None:
    cursor = st.session_state.get("orders_summary_after_case_no")
    cached = _cached_order_summary(cursor) or {}
    ...
    if previous_column.button("上一頁訂單", disabled=not history):
        st.session_state["orders_summary_after_case_no"] = history.pop()
        st.rerun()
    page_column.caption(f"訂單摘要第 {len(history) + 1} 頁，每頁最多 50 筆")
    next_cursor = cached.get("next_cursor")
    if next_column.button("下一頁訂單", disabled=not next_cursor):
        history.append(cursor)
        st.session_state["orders_summary_after_case_no"] = next_cursor
        st.rerun()


@st.cache_data(ttl=REFERENCE_DATA_CACHE_SECONDS, show_spinner=False)
def _request_list(url, header_items, label):
    response = requests.get(url, headers=dict(header_items), timeout=10)
    response.raise_for_status()
    payload = response.json()
    if not isinstance(payload, dict) or not payload.get("success"):
        raise ValueError(f"取得{label}清單回應狀態不符")
    data = payload.get("data")
    if not isinstance(data, list):
        raise ValueError(f"{label}資料格式非陣列")
    return data

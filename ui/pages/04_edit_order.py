...
def show():
    ...
    # 頂部：選擇單筆訂單與公式鎖定開關
    col_pick, col_lock = st.columns([2.5, 1.5])
    with col_pick:
        order_opts = {
    ...
    with col_lock:
        st.write("🔒 **公式欄位安全鎖定**")
        is_unlocked = st.checkbox("🔓 強制解鎖自訂衍生公式欄位", value=False, key="guardrail_unlock_toggle")
    ...
    with st.container(border=True):
        ...
        with c2:
            ...
            w_service_mode = st.selectbox("休假方式 (每週預設排休)", s_mode_opts, index=mode_idx, key=f"v25_mode_{target_oid}")
            w_start_date = st.date_input("預期服務開始日", value=safe_date(target_order.get('start_date')), key=f"v25_st_{target_oid}")
        ...
        with c3:
            ...
            if not is_unlocked:
                ...
                w_act_end = st.date_input("服務結束 (🔓 自訂)", value=safe_date(target_order.get('actual_end_date') or calc_act_end), key=f"v25_act_end_custom_{target_oid}")
    ...
    with st.container(border=True):
        ...
        with mc1:
            w_floor_fee = st.number_input("樓層費用 (元)", value=safe_int(target_order.get('floor_fee', 0)), step=100, key=f"v25_fl_{target_oid}")
            w_employer_rate = st.number_input("雇主單價 (元/天)", value=safe_int(target_order.get('employer_hourly_rate', 2000)), step=100, key=f"v25_emp_rate_{target_oid}")
    ...
    with st.container(border=True):
        ...
        with rc1:
            w_dep_rec = st.number_input("已收訂金 (元)", value=safe_int(curr_p.get('deposit_received')), step=100, key=f"v25_dep_rec_{target_oid}")
            w_bal_rec = st.number_input("已收尾款 (元)", value=safe_int(curr_p.get('balance_received')), step=100, key=f"v25_bal_rec_{target_oid}")
            ...
            w_order_status = st.selectbox("訂單成立狀態", status_list, index=st_idx, key=f"v25_status_{target_oid}")
        
        with rc2:
            w_notes = st.text_area("備註 (注意事項/備忘)", value=target_order.get('notes') or "", key=f"v25_notes_{target_oid}")
            w_cancel_reason = ""
            if w_order_status == "訂單取消":
                w_cancel_reason = st.text_area("取消原因 (選取訂單取消時強制填寫)", value=target_order.get('cancel_reason') or "", key=f"v25_cancel_rea_{target_oid}")

    st.markdown("---")
    if st.button("💾 確定儲存 36 欄位試算與變更結果", key=f"btn_save_v25_{target_oid}", type="primary"):

...
> = ({
  ...
  const applyPreview = async (): Promise<void> => {
    ...
    try {
      ...
      setMessage(
        ...
        + `取消 ${receipt.cancelledIntentCount} 個 intent、${receipt.cancelledTaskCount} 個 task。`
      );
      onCommitted?.(receipt);
    } catch (error) {
      if (controller.signal.aborted) return;
  ...
  return (
    <section className="line-action-panel" aria-label="LINE 通知規則維護">
      <div className="line-section-heading">
        <div>
          <h4>通知規則維護</h4>
          <p>Current revision：{revision}。每次儲存或刪除都必須重新 Preview。</p>
        </div>
        <button type="button" disabled={busy} onClick={addRule}>新增規則</button>
      </div>

      {draft.rules.length > 0 ? (
        <>
          <label htmlFor="line-notification-rule-selector">要編輯的通知規則</label>
          <select
      ...
      <div className="line-action-row">
        ...
        {activeRule && !baselineHasActiveRule && (
          <button type="button" disabled={busy} onClick={cancelNewRule}>取消新增規則</button>
        )}
      </div>
      {draftChanged && baselineHasActiveRule && (
        <p>有未儲存編輯時，刪除功能會鎖定；先儲存或重新載入後再刪除。</p>
      )}

      {state === 'loading' && <div role="status">正在執行通知規則操作…</div>}
      {previewIntent && (
        <div className="line-preview-result">
          <strong>{previewIntent.kind === 'save' ? '儲存預覽已就緒' : '刪除預覽已就緒'}</strong>
          <p>版本 {previewIntent.preview.beforeRevision} → {previewIntent.preview.resultingRevision}</p>
          <p>套用後規則數：{previewIntent.preview.ruleCount}</p>
          <p>設定指紋摘要：{previewIntent.preview.fingerprintSummary}</p>
      ...
      {state === 'success' && message && <div className="line-success" role="status">{message}</div>}
      {state === 'error' && message && <div className="line-error" role="alert">{message}</div>}
    </section>
  );
};

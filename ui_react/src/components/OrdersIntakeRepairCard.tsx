...
export interface OrdersIntakeRepairCardProps {
  item: Pick<OrderSummaryItem, 'case_no' | 'client_name' | 'order_status' | 'start_date' | 'service_days'>;
  onChanged: () => Promise<void>;
}

export const OrdersIntakeRepairCard: React.FC<OrdersIntakeRepairCardProps> = ({ item, onChanged }) => {
        ...
        <section aria-label="補齊約定服務資料" style={{ display: 'grid', gap: 8 }}>
          ...
          {termsPreview && (
            <div style={{ display: 'grid', gap: 6 }}>
              ...
              <div aria-label="服務資料補件前後">
                <div>補件前：{termsPreview.before_start_date ?? '未填寫'}／{termsPreview.before_service_days ?? '未填寫'} 天</div>
                <div>補件後：{termsPreview.after_start_date}／{termsPreview.after_service_days} 天</div>
              </div>

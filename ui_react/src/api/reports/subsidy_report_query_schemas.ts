 ...
 * Description: 定義季度、年度與營運週報共用的補助分區 canonical strict Zod views。
 */
import { z } from 'zod';
...
export const SubsidyReportRowSchema = z.strictObject({ serial_number: z.number().int().positive(), case_no: z.string().min(1), eligibility: z.string(), service_start: DateSchema, service_end: DateSchema, subsidy_hours: PositiveDecimalTextSchema, subsidy_days: PositiveDecimalTextSchema, service_days: z.number().int().positive(), subsidy_amount_ntd: z.number().int().nonnegative(), unit_price_ntd: z.number().int().nonnegative(), employer_name: z.string(), staff_name: z.string(), identity_card: z.string(), address: z.string().min(1) });

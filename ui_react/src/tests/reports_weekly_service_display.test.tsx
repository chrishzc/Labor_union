 ...
 * Description: 聚焦驗證每週服務工時報表的有資料、無資料與查詢失敗呈現。
 */
...
describe('ReportsPage weekly service display', () => {
  afterEach(() => {
    vi.restoreAllMocks();
  });

  it('選到有服務紀錄的週別時顯示每週服務工時', async () => {
  ...
  it('週報有其他資料但沒有服務工時時明確顯示無資料', async () => {
    ...
    expect(screen.getByText('本週服務工時無資料。')).toBeInTheDocument();
  });

  it('整週沒有可列入資料時明確顯示無資料', async () => {
    vi.spyOn(weeklyOperationsReportQueryClient, 'query').mockResolvedValue({
      ...WEEKLY_OPERATIONS_REPORT,
      case_rows: [],
      subsidy_partitions: [],
  ...
  it('週報查詢失敗時顯示錯誤而不是空白', async () => {
    vi.spyOn(weeklyOperationsReportQueryClient, 'query').mockRejectedValue(new Error('週報查詢失敗'));

    render(<ReportsPage />);

    const alert = await screen.findByRole('alert');
    expect(alert).toHaveTextContent('週報查詢失敗');

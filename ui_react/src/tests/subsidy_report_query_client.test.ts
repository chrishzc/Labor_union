 ...
 * Description: 驗證補助報表client的fresh Session、GET、完整partition、aggregate與canonical PII契約。
 */
...
describe('subsidy report query client', () => {
  ...
  it('accepts canonical PII and rejects aggregate mismatch', async () => {
    const row = SUBSIDY_REPORT_RESPONSE.data.partitions[0].rows[0];
    globalThis.fetch = vi.fn().mockResolvedValueOnce(response({ ...SUBSIDY_REPORT_RESPONSE, data: { ...SUBSIDY_REPORT_RESPONSE.data, partitions: [{ ...SUBSIDY_REPORT_RESPONSE.data.partitions[0], rows: [{ ...row, identity_card: 'A123456789' }] }, SUBSIDY_REPORT_RESPONSE.data.partitions[1]] } })).mockResolvedValueOnce(response({ ...SUBSIDY_REPORT_RESPONSE, data: { ...SUBSIDY_REPORT_RESPONSE.data, total_amount_ntd: 1 } }));
    await expect(subsidyReportQueryClient.query({ kind: 'quarterly', applicationYear: 2026, quarter: 1 })).resolves.toBeDefined();

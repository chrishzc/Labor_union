...
function crossMonthReport() {
  ...
  report.summary = {
    ...
    order_status_counts: { ...report.summary.order_status_counts, '服務中': 3 }, order_status_missing_count: 0,
  };
  ...
  report.monthly_subtotals = [
    { ...report.summary, year: 2026, month: 8, start_date: '2026-08-31', end_date: '2026-09-06',
      ...
      order_status_counts: { ...report.summary.order_status_counts, '服務中': 2 },
      promotion_count: 12, inquiry_count: 34, review_rejected_count: 0 },
    { ...report.summary, year: 2026, month: 9, start_date: '2026-09-07', end_date: '2026-09-13',
      ...
      order_status_counts: { ...report.summary.order_status_counts, '服務中': 1 },
      promotion_count: 5, inquiry_count: 0, review_rejected_count: 0 },
  ];
  report.annual_totals = [{
    ...
    order_status_counts: { ...report.summary.order_status_counts, '服務中': 10 },
    promotion_count: 77, inquiry_count: 99, review_rejected_count: 0,
  }];
  return report;
}
...
describe('週報年度累計與月小計', () => {
  ...
  it('案件為空仍保留每週數值及月小計', async () => {
    const report = crossMonthReport();
    report.case_rows = [];
    const zeroStatuses = Object.fromEntries(Object.keys(report.summary.order_status_counts).map((status) => [status, 0]));
    report.summary = { ...report.summary, application_count: 0, general_eligible_count: 0, order_established_count: 0, order_status_counts: zeroStatuses };
    report.monthly_subtotals = report.monthly_subtotals.map((total) => ({
      ...total, application_count: 0, general_eligible_count: 0, order_established_count: 0, order_status_counts: zeroStatuses,
    }));
    const table = await renderReport(report);
  ...
  it('所有訂單狀態分別顯示，審核不符合及無訂單另計，沿用 server counts', async () => {
    const report = crossMonthReport();
    const counts = {
      '待補件': 1, '洽談中': 2, '訂單成立': 3, '服務中': 4, '訂單完成': 5, '訂單取消': 6,
      '歷史訂單－未服務': 7, '歷史訂單－服務中': 8, '歷史訂單－服務完成': 9, '歷史訂單－帳務完成': 10,
    };
    report.annual_totals[0] = {
      ...report.annual_totals[0], application_count: 56,
      order_status_counts: counts, order_status_missing_count: 1, review_rejected_count: 2,
    };
    const table = await renderReport(report);
    const annual = within(table).getByRole('row', { name: '115年度累計' });
    ...
    expect(within(annual).getByText('無訂單／狀態缺值：1')).toBeInTheDocument();
    expect(within(annual).getByText('審核不符合：2')).toBeInTheDocument();
    const august = within(table).getByRole('row', { name: '115年8月小計' });
    expect(within(august).getByText('服務中：2')).toBeInTheDocument();
    expect(within(august).getByText('訂單成立：0')).toBeInTheDocument();
  });

  it('strict schema 拒絕缺少及負數狀態計數', () => {
    const report = crossMonthReport();
    report.annual_totals[0].order_status_counts['待補件'] = -1;
    expect(WeeklyOperationsReportSchema.safeParse(report).success).toBe(false);
    const missing = structuredClone(WEEKLY_OPERATIONS_REPORT) as unknown as Record<string, unknown>;
    const summary = missing.summary as Record<string, unknown>;
    delete summary.order_status_counts;
    expect(WeeklyOperationsReportSchema.safeParse(missing).success).toBe(false);
  });

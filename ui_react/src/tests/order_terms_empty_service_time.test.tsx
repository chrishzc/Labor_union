...
function queryFixture() {
  return OrderTermsQuerySchema.parse({
    ...
    assignments: [],
    terms: {
      planned_start_date: '2026-10-01', service_days: 5,
      service_hours_per_day: 8, requires_cooking: null, floor_fee_ntd: 0,
...
beforeEach(() => {
  ...
  vi.mocked(orderTermsMutationClient.preview).mockImplementation(async (_caseNo, payload) => {
    ...
    return OrderTermsPreviewSchema.parse({
      before: current.terms, after: parsed.proposed_terms,
      order_version: 3, scheduling_version: 2, scheduling_generation: 1,
...
describe('Issue #337 empty service time', () => {
  it('accepts explicit all-null input while rejecting a partial tuple', () => {
    ...
    expect(OrderTermsInputSchema.safeParse({
      ...input, service_time: { ...emptyTime, start_time: '09:00:00' },
    }).success).toBe(false);
  });
  ...
  it('still blocks a half-filled time tuple before sending preview', () => {
    const query = queryFixture();
    render(<OrderTermsMutationPanel caseNo={query.case_no} query={query} />);
  ...
  it('still calculates half-hour durations when the user fills the time fields', async () => {
    const query = queryFixture();
    render(<OrderTermsMutationPanel caseNo={query.case_no} query={query} />);
    fireEvent.change(screen.getByLabelText('Beta 每日開始時間'), { target: { value: '09:00' } });
    ...
    expect(proposed.service_time).toEqual({ start_time: '09:00:00', end_time: '13:30:00', end_day_offset: 0 });
  });

  it('does not recalculate saved hours or truncate saved seconds on an unrelated edit', async () => {
    const query = queryFixture();
    query.terms.service_hours_per_day = 7.5;
    query.terms.service_time = { start_time: '09:00:30', end_time: '17:00:30', end_day_offset: 0 };
    render(<OrderTermsMutationPanel caseNo={query.case_no} query={query} />);
    fireEvent.change(screen.getByLabelText('Beta 計畫服務開始日'), { target: { value: '2026-10-02' } });
    fireEvent.click(previewButton());
    expect(await submittedTerms()).toEqual({ ...query.terms, planned_start_date: '2026-10-02' });
  ...
  it('reads an all-null result back and permits another edit after saving', async () => {
    const query = queryFixture();
    const refreshed = { ...query, order_version: 4, terms: { ...query.terms, planned_start_date: '2026-10-02' } };
    ...
    vi.mocked(orderTermsMutationClient.apply).mockResolvedValue(OrderTermsReceiptSchema.parse({
      case_no: query.case_no, order_version: 4, scheduling_version: 2, scheduling_generation: 1,
      client_finance_version: 4, payroll_version: 5, lifecycle_status: '洽談中',
      service_data_lock_formed: false, cancelled_assignment_ids: [], created_assignment_keys: [],
      official_service_day_count: 0, official_service_hours: 0, preview_fingerprint: 'a'.repeat(64),
    }));
    render(<OrderTermsMutationPanel caseNo={query.case_no} query={query} onObserved={onObserved} />);
    fireEvent.change(screen.getByLabelText('Beta 計畫服務開始日'), { target: { value: '2026-10-02' } });
    fireEvent.click(previewButton());
    // Existing reason/receipt behavior is not changed by #337; #339 owns its removal.
    fireEvent.change(await screen.findByLabelText('Beta 條款變更原因'), { target: { value: 'test date correction' } });

 ...
 * Description: 驗證 LINE 訂單群組三個 GET、direct strict response、identity fail closed 與顯示去識別。
 */
...
describe('LINE order group query client', () => {
  ...
  it('查詢 list/detail/events 並以 adapter 遮蔽 group、actor 與 fingerprint', async () => {
    ...
    const page = await lineOrderGroupQueryClient.list({ status: 'active', limit: 20 });
    const detail = await lineOrderGroupQueryClient.detail(record.case_no);
    const events = await lineOrderGroupQueryClient.events(record.case_no, 10);

    expect(fetchMock.mock.calls.map((call) => call[0])).toEqual([
      '/api/v1/line/order-groups?status=active&limit=20',
      '/api/v1/line/order-groups/CASE-2026-001',
      '/api/v1/line/order-groups/CASE-2026-001/events?limit=10',
    ]);
    ...
    const rendered = JSON.stringify([adaptLineOrderGroupRecord(page.items[0]), adaptLineOrderGroupRecord(detail), adaptLineOrderGroupEvent(events[0])]);
  ...
  it('拒絕 extra schema drift 與 detail/event case identity 漂移', async () => {
    globalThis.fetch = vi.fn().mockResolvedValue(response({ items: [{ ...record, invitation_url: 'raw-secret' }], total: 1 }));
    ...
    globalThis.fetch = vi.fn().mockResolvedValue(response([{ ...event, case_no: 'OTHER' }]));
  ...
  it('未登入、空白 caseNo 與超界 limit 在 request 前 fail closed', async () => {
    ...
    await expect(lineOrderGroupQueryClient.events(record.case_no, 201)).rejects.toMatchObject({ code: 'LINE_ORDER_GROUP_VALIDATION' });

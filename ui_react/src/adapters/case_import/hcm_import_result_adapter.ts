...
export function adaptHcmImportResult(record: HcmImportResultRecord): HcmImportResultViewModel {
  const total = record.inserted_count + record.inserted_with_warning_count + record.exact_replay_count + record.review_required_count + record.failed_count + record.skipped_existing_count;
  ...
  return {
    ...
    summary: `新增 ${record.inserted_count + record.inserted_with_warning_count}｜問題 ${record.review_required_count + record.inserted_with_warning_count + record.failed_count}｜跳過 ${record.skipped_existing_count}`,
